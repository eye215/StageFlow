...
function ImportPanel({ workspace, production, scenes, castMembers, text, setText, rows, setRows, analyze, analyzeWithAI, save, readPdf, readSpreadsheet, undo, loading, aiAnalyzing, pdfExtractionReport }) {
  ...
  const uploadSuggestions = [
    ...
    { label: '소품·의상표', description: 'IN·OUT과 체인지 정보를 연결해요', ready: rows.some((row) => row.props?.length || row.costumes?.length) },
  ...
  return <section className="import-panel">
    ...
    <section className="import-next-guide"><div><span>NEXT UPLOAD</span><h3>자동연결에 어떤 자료를 더 넣을까요?</h3><p>있는 자료만 추가하면 기존 데이터는 지우지 않고 연결 정보가 채워져요.</p></div><div>{uploadSuggestions.map((item) => <article className={item.ready ? 'ready' : ''} key={item.label}>{item.ready ? <CheckCircle2 /> : <Plus />}<span><b>{item.label}</b><small>{item.ready ? '연결됨' : item.description}</small></span></article>)}</div></section>
