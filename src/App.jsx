...
async function functionErrorMessage(error, data, fallback) {
  ...
  if (response && typeof response.clone === 'function') {
    try {
      ...
      } catch { /* 응답 본문이 이미 소비된 경우 SDK 메시지를 사용합니다. */ }
    }
  }
...
export default function App() {
  ...
  const [castForm, setCastForm] = useState({ name: '', gender: '미지정', pairGroup: '', roleName: '', subRoleName: '', type: '주연', notes: '' })
  const [showCastForm, setShowCastForm] = useState(false)
  const [propItems, setPropItems] = useState([])
  const [propForm, setPropForm] = useState({ name: '', kind: '소품', sceneNos: [], inBy: '', outBy: '', note: '' })
  ...
  useEffect(() => {
    ...
    // 각 세션 렌더의 로더를 한 번만 실행합니다. 로더 내부는 현재 세션을 캡처합니다.
    ...
    try {
      ...
      if (inviteToken) {
        ...
        if (joinError) {
          ...
          const joinedLookup = await supabase.from('productions').select('id, workspace_id').eq('id', joinedResultId).maybeSingle()
          if (joinedLookup.error && !/0 rows|not found/i.test(joinedLookup.error.message || '')) {
            throw new Error(`초대 공연 확인 실패: ${joinedLookup.error.message}`)
          }
          let joinedRow = joinedLookup.data || null
          // 구버전 RPC는 production_id 대신 workspace_id를 반환했습니다. 링크에 포함된
          // 공연 ID가 실제 접근 가능하고 반환된 workspace에 속할 때만 호환 경로로 사용합니다.
          if (!joinedRow && requestedProductionId) {
            ...
            if (fallback.error && !/0 rows|not found/i.test(fallback.error.message || '')) {
              throw new Error(`초대 링크 공연 확인 실패: ${fallback.error.message}`)
            }
            joinedRow = fallback.data?.workspace_id === joinedResultId ? fallback.data : null
          }
          if (joinedRow && requestedProductionId && joinedRow.id !== requestedProductionId) {
            joinedRow = null
            setNotice('초대 토큰과 공연 정보가 일치하지 않아요. 해당 공연에서 새 초대 링크를 만들어주세요.')
          }
      ...
      let workspaceQuery = supabase
        ...
        .limit(1)
      if (joinedWorkspaceId) workspaceQuery = workspaceQuery.eq('workspace_id', joinedWorkspaceId)
  ...
  async function loadProductions(workspaceId, preferredProductionId = '') {
    ...
    if (membershipUnavailable) {
      ...
      setNotice('공연별 접근 DB가 아직 연결되지 않았어요. production_scoped_access.sql을 먼저 적용해주세요.')
      return
    }
  ...
  async function loadHomeOverview(productionId) {
    ...
    if (castResult.data) {
      try {
        ...
        homeCast = Array.isArray(payload.members)
  ...
  async function deleteProduction(id, options = {}) {
    ...
    try {
      const { data: membership, error: membershipError } = await supabase
        ...
        .eq('user_id', session.user.id)
        .maybeSingle()
      const targetProduction = productions.find((item) => item.id === id) || (selected?.id === id ? selected : null)
      if (membershipError && !isMissingBackendObject(membershipError, 'production_members')) throw new Error(`삭제 권한 확인 실패: ${membershipError.message}`)
      const isOwner = membership?.role === 'owner' || (isMissingBackendObject(membershipError, 'production_members') && targetProduction?.created_by === session.user.id)
  ...
  async function clearProductionUploads(productionId, selectedFolders = ['imports', 'music', 'materials']) {
    ...
    setBusy(true)
    try {
      if (folders.includes('music')) {
        musicOrderGenerationRef.current += 1
        await musicOrderSaveRef.current.catch(() => {})
      }
      const groups = await Promise.all(folders.map((folder) => listStorageFilesRecursive(`${workspace.id}/${productionId}/${folder}`)))
      const uploadedPaths = groups.flat()
      const controlPaths = folders.includes('music') ? [`${workspace.id}/${productionId}/data/music-order.json`] : []
      const paths = [...new Set([...uploadedPaths, ...controlPaths])]
      if (paths.length) await removeStoragePaths(paths)
      let cleanupWarning = ''
      if (folders.includes('music')) {
        const { error: playbackError } = await supabase.from('production_playback').delete().eq('production_id', productionId)
        if (playbackError && !isMissingBackendObject(playbackError, 'production_playback')) cleanupWarning = ` 공유 재생 상태는 정리하지 못했어요: ${playbackError.message}`
        setPendingMusic([])
        setMusicByScene({})
      }
      setNotice(uploadedPaths.length ? `업로드 자료 ${uploadedPaths.length}개를 초기화했어요.${cleanupWarning}` : `초기화할 업로드 자료가 없어요.${cleanupWarning}`)
      return true
    } catch (error) {
  ...
  async function createScene(event) {
    ...
    if (!Number.isInteger(sceneNo) || sceneNo < 0) return setNotice('장면 번호를 0 이상의 정수로 입력해 주세요.')
  ...
  async function updateScene(id, values) {
    ...
    if (!Number.isInteger(sceneNo) || sceneNo < 0) { setNotice('장면 번호를 0 이상의 정수로 입력해 주세요.'); return false }
  ...
  async function reorderScene(sceneId, direction) {
    const ordered = [...scenes].sort((a, b) => Number(a.sort_order || 0) - Number(b.sort_order || 0) || Number(a.scene_no) - Number(b.scene_no))
    const fromIndex = ordered.findIndex((scene) => scene.id === sceneId)
    const toIndex = fromIndex + Number(direction)
    if (fromIndex < 0 || toIndex < 0 || toIndex >= ordered.length) return false
    const [moved] = ordered.splice(fromIndex, 1)
    ordered.splice(toIndex, 0, moved)
    setBusy(true)
    try {
      const results = await Promise.all(ordered.map((scene, index) => supabase.from('scenes').update({ sort_order: index }).eq('id', scene.id)))
      const failed = results.find(({ error }) => error)
      if (failed?.error) throw failed.error
      await loadScenes(selected.id)
      setNotice(`${moved.scene_no}. ${moved.title} 장면 순서를 변경했어요.`)
      return true
    } catch (error) {
      await loadScenes(selected.id)
      setNotice(`장면 순서 변경 실패: ${error.message}`)
      return false
    } finally {
      setBusy(false)
    }
        ...
        try {
          for (let index = 0; index < ocrPageNumbers.length; index += 1) {
            ...
            try {
              ...
              if (!context) throw new Error('이 기기에서 PDF OCR용 화면을 만들 수 없어요.')
  ...
  async function persistImportedSoundtracks(importedRows, productionId = selected.id) {
    ...
    const songIdentity = (code, title) => `${normalizeMatch(code)}::${normalizeMatch(title)}`
    const uniqueRecords = new Map()
    soundtrackRows.forEach(({ row, track, index }) => {
      const sceneId = sceneIds.get(Number(row.number))
      if (!sceneId) return
      const code = String(track.code || `SONG.${row.number}`).trim()
      const title = String(track.title || row.title || `SONG ${row.number}`).trim()
      const identity = songIdentity(code, title)
      if (!uniqueRecords.has(identity)) uniqueRecords.set(identity, {
        production_id: productionId,
        scene_id: sceneId,
    ...
    const songIds = new Map((savedSongs || []).map((song) => [songIdentity(song.code, song.title), song.id]))
    const relations = soundtrackRows.map(({ row, track, index }) => {
      const sceneId = sceneIds.get(Number(row.number))
      const code = String(track.code || `SONG.${row.number}`).trim()
      const title = String(track.title || row.title || `SONG ${row.number}`).trim()
      const songId = songIds.get(songIdentity(code, title))
      return sceneId && songId ? {
        production_id: productionId,
        scene_id: sceneId,
        song_id: songId,
        sort_order: index,
    ...
    if (relations.length) {
      const { error: relationError } = await supabase.from('scene_songs').upsert(relations, { onConflict: 'scene_id,song_id' })
      if (relationError) {
        if (!/scene_songs|schema cache|relation/i.test(relationError.message || '')) console.error('Scene-song relation save failed', relationError)
        return {
          ok: false,
          message: /scene_songs|schema cache|relation/i.test(relationError.message || '')
            ? '장면↔음악 연결 DB 미적용'
            : `장면↔음악 연결 저장 실패: ${relationError.message}`,
        }
      }
    }
    return { ok: true, count: relations.length }
  }
    ...
    try {
      const uploadOne = async (item) => {
        try {
          ...
          failures.push(`${item.file.name} (${formatBytes(item.file.size)}): ${error.message}`)
        }
      }
      // 모바일 메모리와 속도의 균형을 위해 최대 두 파일만 동시에 전송합니다.
      for (let index = 0; index < matched.length; index += 2) {
        await Promise.all(matched.slice(index, index + 2).map(uploadOne))
      }
      setPendingMusic((items) => items.filter((item) => !uploadedItems.includes(item)))
      let cueWarning = ''
      if (uploadedItems.length) {
        try { await linkUploadedMusicCues(uploadedItems) }
        catch (error) { cueWarning = ` 음악은 저장됐지만 큐 자동 연결 일부가 실패했어요: ${error.message}` }
      }
      await loadMusic(productionId, scenes)
      if (uploaded === matched.length) setNotice(`${uploaded}개 음악파일을 넘버별로 저장했어요.${cueWarning || ' 음향 큐도 연결했어요.'}`)
      else if (failures.length) setNotice(`${uploaded}개 저장 · ${failures.length}개 실패. 실패한 파일은 목록에 남겨뒀어요: ${failures.join(' / ')}${cueWarning}`)
    } catch (error) {
  ...
  async function autoLinkProductionCues() {
    ...
    try {
      ...
      for (const scene of scenes) {
        ...
        if (summary !== String(scene.summary || '')) {
          const { error } = await supabase.from('scenes').update({ summary }).eq('id', scene.id)
          if (error) failures.push(`${scene.scene_no}. ${scene.title}: ${error.message}`)
          else added += Math.max(0, after - before)
        }
      }
      await loadScenes(selected.id)
      if (failures.length) setNotice(`${added}개 큐를 연결했지만 ${failures.length}개 장면은 저장하지 못했어요: ${failures.slice(0, 2).join(' / ')}`)
      else setNotice(added ? `${added}개 큐를 대본·음악·배역에서 찾아 장면에 자동 연결했어요.` : '새로 연결할 큐가 없어요. 기존 연결을 유지했어요.')
    } catch (error) {
      setNotice(`큐 자동 연결 실패: ${error.message || '네트워크 연결을 확인해주세요.'}`)
    } finally {
      setBusy(false)
    }
  }
  ...
  async function loadMusic(productionId, sceneList = scenes) {
    ...
    const entries = await Promise.all(sceneList.map(async (scene) => {
      ...
      let signedRows = []
      try {
        if (files.length) signedRows = await createSignedStorageUrls(files.map((file) => file.path), 604800)
      } catch (error) {
  ...
  function queueMusicOrderSave(productionId, musicState) {
    const generation = musicOrderGenerationRef.current
    const order = Object.fromEntries(Object.entries(musicState).map(([number, items]) => [String(number), items.map((item) => item.path)]))
    const path = `${workspace.id}/${productionId}/data/music-order.json`
    const save = async () => {
      if (generation !== musicOrderGenerationRef.current) return false
    ...
    const queued = musicOrderSaveRef.current.then(save, save)
    musicOrderSaveRef.current = queued.catch(() => {})
    return queued
  }
  ...
  async function reorderMusic(sceneNo, fromPath, toPath) {
    ...
    const productionId = selected.id
    try {
      await queueMusicOrderSave(productionId, next)
    } catch (error) {
      if (selectedIdRef.current === productionId) await loadMusic(productionId, scenes)
  ...
  async function moveMusicFile(file, sourceSceneNo, targetSceneNo) {
    const sourceNumber = Number(sourceSceneNo)
    const targetNumber = Number(targetSceneNo)
    if (!file?.path || !Number.isFinite(targetNumber) || sourceNumber === targetNumber) return false
    if (!scenes.some((scene) => Number(scene.scene_no) === targetNumber)) {
      setNotice('이동할 장면을 다시 선택해주세요.')
      return false
    }
    setUploadingMusic(true)
    const productionId = selected.id
    const storedName = String(file.path).split('/').at(-1)
    const targetPath = `${workspace.id}/${productionId}/music/${targetNumber}/${storedName}`
    try {
      const { error } = await supabase.storage.from('stageflow-files').move(file.path, targetPath)
      if (error) throw error
      const next = {
        ...musicByScene,
        [sourceNumber]: (musicByScene[sourceNumber] || []).filter((item) => item.path !== file.path),
        [targetNumber]: [...(musicByScene[targetNumber] || []), { ...file, path: targetPath }],
      }
      let orderWarning = ''
      try { await queueMusicOrderSave(productionId, next) }
      catch (orderError) { orderWarning = ` 순서 정보는 저장하지 못했어요: ${orderError.message}` }
      const { error: playbackError } = await supabase
        .from('production_playback')
        .update({ file_path: targetPath, scene_no: targetNumber })
        .eq('production_id', productionId)
        .eq('file_path', file.path)
      await loadMusic(productionId, scenes)
      setNotice((playbackError && !isMissingBackendObject(playbackError, 'production_playback'))
        ? `음악은 ${targetNumber}번 장면으로 옮겼지만 공유 재생 상태는 갱신하지 못했어요: ${playbackError.message}`
        : `${cleanStoredFileName(file.name)} 음악을 ${targetNumber}번 장면으로 옮겼어요.${orderWarning}`)
      return true
    } catch (error) {
      await loadMusic(productionId, scenes)
      setNotice(`음악 장면 변경 실패: ${error.message}`)
      return false
    } finally {
      setUploadingMusic(false)
    }
  ...
  async function addCastMember(event) {
    ...
    setBusy(true)
    const member = normalizeCastAssignment({ id: crypto.randomUUID(), entityType: 'cast_assignment', ...castForm, name: actorName, pairGroup, roleName, subRoleName: castForm.subRoleName.trim(), notes: castForm.notes.trim(), sceneNumbers: [] })
  ...
  async function renameCastPair(previous, nextName) {
    ...
    if (saved) setNotice(`${previous}를 ${value}(으)로 변경했어요.`)
    return saved
  }

  async function removeCastPair(name) {
    if (castMembers.some((member) => normalizeMatch(member.pairGroup) === normalizeMatch(name))) {
      setNotice('캐스팅이 연결된 페어는 먼저 캐스팅에서 변경하거나 삭제해 주세요.')
      return false
    }
    return persistCastData(castMembers, castPairs.filter((pair) => normalizeMatch(pair) !== normalizeMatch(name)))
  }

  async function addCastRosterActor(pairName, actorName) {
    const pairGroup = String(pairName || '').trim()
    const name = String(actorName || '').trim()
    if (!pairGroup || !name) return false
    const duplicate = castMembers.some((member) => isActorRoster(member) && canonicalActor(member.name) === canonicalActor(name)
      && normalizeMatch(member.pairGroup) === normalizeMatch(pairGroup))
    if (duplicate) {
      setNotice(`${pairGroup}에 ${name} 배우가 이미 있어요.`)
      return false
    }
    const actor = normalizeCastAssignment({ id: crypto.randomUUID(), entityType: 'actor_roster', name, pairGroup, roleName: '', notes: '' })
    const nextPairs = castPairs.some((pair) => normalizeMatch(pair) === normalizeMatch(pairGroup)) ? castPairs : [...castPairs, pairGroup]
    const saved = await persistCastData([...castMembers, actor], nextPairs)
    if (saved) setNotice(`${pairGroup} 배우 명단에 ${name}을(를) 추가했어요.`)
    return saved
  }

  async function removeCastRosterActor(id) {
    const actor = castMembers.find((member) => member.id === id && isActorRoster(member))
    if (!actor) return false
    const assigned = castMembers.some((member) => isCastAssignment(member)
      && canonicalActor(member.name) === canonicalActor(actor.name)
      && normalizeMatch(member.pairGroup) === normalizeMatch(actor.pairGroup))
    if (assigned) {
      setNotice('배역이 연결된 배우는 캐스팅을 먼저 삭제해 주세요.')
      return false
    }
    return persistCastData(castMembers.filter((member) => member.id !== id))
  }

  async function assignCastRole({ roleName, subRoleName = '', type = '주연', sceneNumbers = [], pairGroup, actorName }) {
    const role = String(roleName || '').trim()
    const pair = String(pairGroup || '').trim()
    const actor = String(actorName || '').trim()
    if (!role || !pair || !actor) return false
    const duplicate = castMembers.some((member) => isCastAssignment(member)
      && normalizeMatch(member.roleName) === normalizeMatch(role)
      && normalizeMatch(member.subRoleName) === normalizeMatch(subRoleName)
      && normalizeMatch(member.pairGroup) === normalizeMatch(pair)
      && canonicalActor(member.name) === canonicalActor(actor))
    if (duplicate) {
      setNotice(`${role} · ${pair} · ${actor} 캐스팅은 이미 연결되어 있어요.`)
      return false
    }
    const roster = castMembers.find((member) => isActorRoster(member) && canonicalActor(member.name) === canonicalActor(actor)
      && normalizeMatch(member.pairGroup) === normalizeMatch(pair))
    const assignment = normalizeCastAssignment({
      id: crypto.randomUUID(), entityType: 'cast_assignment', name: actor, pairGroup: pair,
      roleName: role, subRoleName, type, gender: roster?.gender || '미지정',
      notes: '배역 캐스팅에서 연결', sceneNumbers,
    })
    const next = roster ? [...castMembers, assignment] : [
      ...castMembers,
  ...
  async function importCastFromScenes() {
    ...
    if (!added && !linked) return setNotice('장면에서 새로 가져오거나 연결할 배역이 없어요.')
    setBusy(true)
    if (await persistCastData(next)) {
      const result = [added ? `새 배역 후보 ${added}개` : '', linked ? `기존 캐스팅 등장 장면 ${linked}개` : ''].filter(Boolean).join(' · ')
      setNotice(`${result}를 정리했어요. 배역별로 페어 배우를 선택해 주세요.`)
    }
    setBusy(false)
  }
  ...
  async function consolidateCastDuplicates(options = null) {
    ...
    const expandedMembers = castMembers.flatMap((member) => {
      ...
      return roles.map((roleName, index) => ({
        ...
        roleName,
        sceneNumbers: matchScenesForSplitRole(member.name, roleName, scenes, member.sceneNumbers || []),
      }))
    })
    for (const member of expandedMembers) {
      const key = [member.entityType, normalizeMatch(member.roleName || member.name), normalizeMatch(member.subRoleName), normalizeMatch(member.pairGroup), canonicalActor(member.name)].join('::')
    ...
    const { error: backupError } = await supabase.storage.from('stageflow-files').upload(castCleanupBackupPath(selected.id), backupBody, { upsert: true, contentType: 'application/json' })
    if (backupError) {
      setBusy(false)
      setNotice(`이름 정리 백업 실패: ${backupError.message}`)
      return 0
    }
    const saved = await persistCastData(next)
    setBusy(false)
  ...
  async function undoCastNameCleanup() {
    setBusy(true)
    const { data, error } = await supabase.storage.from('stageflow-files').download(castCleanupBackupPath(selected.id))
    if (error || !data) {
      setBusy(false)
      setNotice('되돌릴 이름 정리 백업이 없어요.')
      return false
    }
    try {
      const backup = JSON.parse(await data.text())
      if (!Array.isArray(backup.members)) throw new Error('백업 형식이 올바르지 않아요.')
      const saved = await persistCastData(backup.members)
      if (saved) setNotice('마지막 이름 정리 전 배우·배역 상태로 복원했어요.')
      setBusy(false)
      return saved
    } catch (error) {
      setBusy(false)
      setNotice(`이름 정리 복원 실패: ${error.message}`)
      return false
    }
  }
  ...
  async function addPropItem(event) {
    ...
    const selectedSceneNos = [...new Set((propForm.sceneNos || []).map(Number))]
      .filter((sceneNo) => Number.isFinite(sceneNo) && scenes.some((scene) => Number(scene.scene_no) === sceneNo))
      .sort((a, b) => a - b)
    setBusy(true)
    const targets = selectedSceneNos.length ? selectedSceneNos : [null]
    const items = targets.map((sceneNo) => ({
      id: crypto.randomUUID(),
      name: propForm.name.trim(),
      kind: propForm.kind === '대도구' ? '대도구' : '소품',
      sceneNo,
      inBy: String(propForm.inBy || '').trim(),
      outBy: String(propForm.outBy || '').trim(),
      note: String(propForm.note || '').trim(),
      ready: false,
    }))
    if (await persistPropData([...propItems, ...items])) {
      setPropForm({ name: '', kind: '소품', sceneNos: [], inBy: '', outBy: '', note: '' })
      setShowPropForm(false)
      setNotice(`${propForm.name.trim()} 항목을 ${selectedSceneNos.length ? `${selectedSceneNos.length}개 장면에` : '장면 미지정으로'} 등록했어요.`)
    }
    setBusy(false)
  }
  ...
  if (selected) return (
    <ProductionView
      ...
      updateScene={updateScene} deleteScene={deleteScene} reorderScene={reorderScene} showForm={showSceneForm} setShowForm={setShowSceneForm}
      ...
      castPairs={castPairs} addCastPair={addCastPair} renameCastPair={renameCastPair} removeCastPair={removeCastPair}
      addCastRosterActor={addCastRosterActor} removeCastRosterActor={removeCastRosterActor} assignCastRole={assignCastRole}
      showCastForm={showCastForm} setShowCastForm={setShowCastForm}
      addCastMember={addCastMember} updateCastMember={updateCastMember} removeCastMember={removeCastMember} toggleCastScene={toggleCastScene} toggleCastChoreography={toggleCastChoreography} importCastFromScenes={importCastFromScenes}
      consolidateCastDuplicates={consolidateCastDuplicates}
      undoCastNameCleanup={undoCastNameCleanup}
...
function HomeDashboardV2({ session, workspace, productions, defaultProduction, daysLeft, progress, scenes, musicCount, musicByScene, propStats, tasks = [], events = [], castMembers, propItems, openAt, profileOpen, setProfileOpen, chooseDefaultProduction, notice, showForm, setShowForm, productionForm, setProductionForm, createProduction, busy, createTeamInvite, showRoleClaim, inviteCastMembers, claimInviteRole }) {
  ...
  const myTasks = tasks.filter((task) => task.assigneeUserId === session.user.id || assignmentMatches(task.assignee, myCast))
  const scopedTasks = taskScope === 'mine' ? myTasks : tasks
  ...
  return <div className="app-shell home-v2">
    ...
    <main className="content home-dashboard-v2">
      ...
      {defaultProduction ? <>
        ...
        <section className="home-task-block"><div className="compact-heading"><div><span>TO DO</span><h2>공연 준비 할 일</h2></div><button className="text-button" onClick={() => openAt('tasks')}>{scopedTasks.filter((task) => !task.done).length}개 남음</button></div><div className="home-task-scope"><button className={taskScope === 'mine' ? 'active' : ''} onClick={() => setTaskScope('mine')}>내 할 일 {myTasks.filter((task) => !task.done).length}</button><button className={taskScope === 'all' ? 'active' : ''} onClick={() => setTaskScope('all')}>팀 전체 {tasks.filter((task) => !task.done).length}</button></div>{pendingTasks.length ? <div className="home-task-list">{pendingTasks.map((task) => <button key={task.id} onClick={() => openAt('tasks')}><CheckCircle2 /><div><strong>{task.title}</strong><span>{task.assignee ? `${task.assignee} · ` : ''}{task.dueDate ? formatTaskDue(task.dueDate) : '마감일 미정'}</span></div><ChevronRight /></button>)}</div> : <div className="clear-state"><CheckCircle2 /><div><strong>{taskScope === 'mine' ? '내게 남은 준비 업무가 없어요' : '남은 준비 업무가 없어요'}</strong><span>{taskScope === 'mine' ? '팀 전체를 누르면 다른 담당 업무도 확인할 수 있어요.' : '할 일 탭에서 새로운 공연 준비 업무를 추가할 수 있어요.'}</span></div></div>}</section>
...
function normalizeCastAssignment(member = {}) {
  ...
  const rawName = String(member.name || member.actorName || '').trim()
  ...
  const autoRolePlaceholder = Boolean(roleName && rawName && !pairGroup && normalizeMatch(roleName) === normalizeMatch(rawName))
  const name = autoRolePlaceholder ? '' : rawName
  const requestedType = String(member.entityType || '')
  const entityType = autoRolePlaceholder
    ? 'role_candidate'
    : ['cast_assignment', 'actor_roster', 'role_candidate'].includes(requestedType)
    ? requestedType
    : roleName && name && pairGroup
      ? 'cast_assignment'
      : roleName && !name
        ? 'role_candidate'
        : name && pairGroup && !roleName
          ? 'actor_roster'
          : 'cast_assignment'
  return {
    ...member,
...
const isCastAssignment = (member) => member?.entityType === 'cast_assignment'
  && Boolean(member.name?.trim() && member.roleName?.trim() && member.pairGroup?.trim())
...
function ensureActorRosterRecords(members = []) {
  ...
  members.filter(isCastAssignment).forEach((assignment) => {
    const exists = result.some((member) => isActorRoster(member)
      && canonicalActor(member.name) === canonicalActor(assignment.name)
      && normalizeMatch(member.pairGroup) === normalizeMatch(assignment.pairGroup))
    if (!exists) result.push(normalizeCastAssignment({
      id: crypto.randomUUID(), entityType: 'actor_roster', name: assignment.name,
      pairGroup: assignment.pairGroup, roleName: '', gender: assignment.gender, notes: '',
    }))
  })
  return result
}
  ...
  const addRoleCandidate = (roleName, type, sceneNo) => {
    ...
    if (roleAssignments.length) {
      roleAssignments.forEach((member) => {
        ...
        member.sceneNumbers.sort((a, b) => a - b)
      })
      return
    }
    const candidate = members.find((member) => isRoleCandidate(member) && normalizeMatch(member.roleName) === normalizeMatch(cleanRole))
    if (candidate) {
      if (!candidate.sceneNumbers.includes(sceneNo)) candidate.sceneNumbers.push(sceneNo)
      candidate.sceneNumbers.sort((a, b) => a - b)
      return
    }
    members.push(normalizeCastAssignment({ id: crypto.randomUUID(), entityType: 'role_candidate', name: '', pairGroup: '', roleName: cleanRole, type, notes: '대본에서 찾은 배역 · 페어별 배우 선택 필요', sceneNumbers: [sceneNo] }))
  }
    ...
    String(scene.summary || '').split('\n').forEach((line) => {
      ...
      splitRoleEntries(match[2]).forEach((entry) => {
        ...
        if (paired) {
          ...
          addRoleCandidate(roleName, type, Number(scene.scene_no))
        } else {
          addRoleCandidate(value, type, Number(scene.scene_no))
        }
      })
    })
  })
...
function ProfileSheet({ session, productions, defaultId, choose, createTeam, close, logout }) {
...
function projectedPlaybackPosition(value, now = Date.now()) {
  const base = Math.max(0, Number(value?.position_seconds || 0))
  if (!value?.is_playing) return base
  const updatedAt = new Date(value.updated_at || 0).getTime()
  return Number.isFinite(updatedAt) ? base + Math.max(0, (now - updatedAt) / 1000) : base
}
...
function useSharedProductionPlayback({ production, session, playlist }) {
  ...
  const publish = useCallback(async (patch) => {
    ...
    const payload = {
      ...
      file_path: patch.file_path ?? currentPlayback?.file_path ?? '',
      file_name: patch.file_name ?? currentPlayback?.file_name ?? '',
      scene_no: patch.scene_no ?? currentPlayback?.scene_no ?? null,
      is_playing: patch.is_playing ?? currentPlayback?.is_playing ?? false,
      position_seconds: patch.position_seconds ?? localPosition,
    ...
    if (saveError) {
      playbackRef.current = currentPlayback
      setPlayback(currentPlayback)
      setError(`공유 재생 연결 실패: ${saveError.message}`)
      return false
    }
    setError('')
    return true
  ...
  useEffect(() => {
    ...
    const desired = projectedPlaybackPosition(playback)
    const align = () => {
...
function ProductionView(props) {
  ...
  const [feedbackInboxStatus, setFeedbackInboxStatus] = useState('')
  ...
  const castingAssignments = useMemo(() => castMembers.filter(isCastAssignment), [castMembers])
  ...
  const loadMyFeedback = useCallback(async () => {
    ...
    try {
      const entries = await listAllStorageEntries(base, { sortBy: { column: 'created_at', order: 'desc' } })
      const files = entries.filter((item) => item.id && /\.json$/i.test(item.name)).slice(0, 50)
      const parsed = await Promise.all(files.map(async (item) => {
        const { data, error } = await supabase.storage.from('stageflow-files').download(`${base}/${item.name}`)
        if (error || !data) return null
        try { return JSON.parse(await data.text()) } catch { return null }
      }))
      setMyFeedback(parsed.filter(Boolean).sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || ''))))
      setFeedbackInboxStatus('')
    } catch (error) {
      setMyFeedback([])
      setFeedbackInboxStatus(`내 피드백을 불러오지 못했어요: ${error.message}`)
    }
  }, [production.id, session.user.id, workspace.id])
  useEffect(() => {
    setShowIndex((index) => runScenes.length ? Math.max(0, Math.min(runScenes.length - 1, index)) : 0)
  }, [runScenes.length, setShowIndex])
  useEffect(() => { void loadMyFeedback() }, [loadMyFeedback])
  useEffect(() => {
    if (briefingMemberId && selectedRunPair && !castingAssignments.some((member) => member.id === briefingMemberId && normalizeMatch(member.pairGroup) === normalizeMatch(selectedRunPair))) {
      setBriefingMemberId('')
      window.localStorage.removeItem(`stageflow-briefing-${production.id}`)
    }
  }, [briefingMemberId, selectedRunPair, castingAssignments, production.id])
  useEffect(() => {
    let active = true
  ...
  async function submitRunFeedback(event) {
    event.preventDefault()
    const entries = feedbackRecipients.filter((member) => feedbackDrafts[canonicalActor(member.name)]?.trim())
    ...
    if (results.some(({ error }) => error)) return setFeedbackStatus('일부 피드백을 저장하지 못했어요.')
    setFeedbackStatus(`${entries.length}명에게 개인 피드백을 전달했어요.`)
    setFeedbackDrafts({})
    setFeedbackRunId('')
  }
  const persistFullRun = useCallback(async (session, completed) => {
  ...
  }, [current, next, persistFullRun, runScenes.length, runSession, setShowIndex])
  const goPreviousDuringRun = useCallback(() => {
    if (!runSession || showIndex <= 0 || runAdvanceRef.current) return
    const now = new Date().toISOString()
    const previousScene = runScenes[showIndex - 1]
    setRunSession((currentSession) => {
      if (!currentSession) return currentSession
      const lastSegment = currentSession.segments.at(-1)
      const segments = lastSegment && Number(lastSegment.sceneNo) === Number(previousScene?.scene_no)
        ? currentSession.segments.slice(0, -1)
        : currentSession.segments
      return { ...currentSession, sceneStartedAt: now, segments }
    })
    setShowIndex((index) => Math.max(0, index - 1))
  }, [runScenes, runSession, setShowIndex, showIndex])
  const appendShowEvent = useCallback(async (event) => {
  ...
  }, [showIndex, showHold, showHoldMessage, showController, showCursorLoaded, showCursorPath, tab, runScenes, appendShowEvent])
  ...
  const sceneDetail = scenes.find((scene) => scene.id === sceneDetailId) || null
  const sceneEditing = scenes.find((scene) => scene.id === sceneEditingId) || null
  ...
  const setupState = {
    ...
    actors: castMembers.some((member) => isActorRoster(member) || isCastAssignment(member)),
    casting: castingAssignments.length > 0,
  ...
  if (editingProduction) return <div className="app-shell production-shell-v2 depth-page-active"><header className="topbar production-topbar"><button className="icon-button" onClick={() => setEditingProduction(false)} aria-label="공연으로 돌아가기"><ChevronLeft /></button><div className="topbar-title"><strong>공연 정보 수정</strong><span>{production.title}</span></div><span className="header-spacer" /></header><main className="content production-content"><EntityEditorPage eyebrow="PRODUCTION" title="공연 기본정보" description="공연명·장소·공연일을 수정합니다." back={() => setEditingProduction(false)}><form className="panel form-grid entity-editor-form" onSubmit={saveProduction}><label><span>공연명</span><input required value={productionDraft.title} onChange={(event) => setProductionDraft({ ...productionDraft, title: event.target.value })} /></label><label><span>공연 장소</span><input value={productionDraft.venue} onChange={(event) => setProductionDraft({ ...productionDraft, venue: event.target.value })} /></label><label><span>공연일</span><input type="date" value={productionDraft.performance_start_date} onChange={(event) => setProductionDraft({ ...productionDraft, performance_start_date: event.target.value })} /></label><button className="primary"><Save size={16} /> 변경 저장</button></form></EntityEditorPage></main></div>
    ...
    <main className="content production-content">
      ...
      {tab === 'scenes' && (showForm
        ...
        : <>
          ...
          <section className="scene-list">{!scenes.length && <Empty icon={<Clapperboard />} title="아직 장면이 없어요" description="첫 장면을 등록해 공연 흐름을 만들어보세요." action={() => setShowForm(true)} />}{!!scenes.length && !visibleScenes.length && <Empty icon={<Search />} title="검색 결과가 없어요" description="다른 검색어나 ACT를 선택해보세요." />}{visibleScenes.map((scene) => { const orderIndex = scenes.findIndex((item) => item.id === scene.id); return <SceneCard key={scene.id} scene={scene} musicFiles={musicByScene[scene.scene_no] || []} open={() => setSceneDetailId(scene.id)} moveUp={() => reorderScene(scene.id, -1)} moveDown={() => reorderScene(scene.id, 1)} canMoveUp={orderIndex > 0} canMoveDown={orderIndex >= 0 && orderIndex < scenes.length - 1} orderLoading={busy} /> })}</section>
        </>)}
      ...
      {tab === 'show' && <RunControl session={runSession} elapsed={runElapsed} history={runHistory} current={current} firstScene={firstRunScene} firstProps={firstRunProps} firstCostumes={firstRunCostumes} pairName={selectedRunPair} start={startFullRun} enabled={showController && Boolean(selectedRunPair) && runFeedbackActors.length > 0} />}
      ...
      {tab === 'show' && runSession && <section className="show-mode run-scene-page">
        {!current ? <Empty icon={<Play />} title="진행할 장면이 없어요" description="장면을 먼저 등록해주세요." action={() => setTab('scenes')} /> : <>
          <label className="briefing-picker"><UserRound /><span>내 배역 브리핑</span><select value={briefingMemberId} onChange={(event) => selectBriefingMember(event.target.value)}><option value="">전체 보기</option>{runPairMembers.map((member) => <option key={member.id} value={member.id}>{member.roleName || '배역 미정'} · {member.name}{member.pairGroup ? ` · ${member.pairGroup}` : ''}</option>)}</select></label>
          <div className="show-operations run-essentials">
            <article><div className="show-section-title"><Users /><strong>등장 배역 · 배우</strong><span>{currentCast.length}</span></div>{currentCast.length ? <div className="show-cast-list">{currentCast.map((member) => <span className={member.groupedMemberIds?.includes(briefingMemberId) ? 'selected' : ''} key={member.id}><b>{member.roleName || '배역 미정'}</b><small>{member.name}{member.pairGroup ? ` · ${member.pairGroup}` : ''}{choreographyStatusForMusic(member, currentMusic)}</small></span>)}</div> : <p>연결된 배우가 없어요.</p>}</article>
            <article><div className="show-section-title"><Shirt /><strong>{briefingMember ? '내 의상 · 체인지' : '현재 의상 · 체인지'}</strong><span>{briefingCurrentCostumes.length}</span></div>{briefingCurrentCostumes.length ? <div className="show-costume-list">{briefingCurrentCostumes.map((item, index) => <div key={`${item.role}-${index}`}><b>{item.role}</b><span>{item.name}</span>{item.note && <small>{item.note}</small>}</div>)}</div> : <p>{briefingMember ? '현재 장면에 내 의상 체인지가 없어요.' : '등록된 의상 체인지가 없어요.'}</p>}</article>
            <article><div className="show-section-title"><Package /><strong>{briefingMember ? '내 소품 업무' : '소품·대도구'}</strong><span>{briefingCurrentProps.filter((item) => item.ready).length}/{briefingCurrentProps.length}</span></div>{briefingCurrentProps.length ? <div className="show-prop-list">{briefingCurrentProps.map((item) => <button className={item.ready ? 'ready' : ''} key={item.id} onClick={() => togglePropReady(item.id)}><CheckCircle2 /><div><b>{item.name}</b><small>IN {item.inBy || '미정'} · OUT {item.outBy || '미정'}</small></div></button>)}</div> : <p>{briefingMember ? '현재 장면에 내 소품 업무가 없어요.' : '연결된 소품이 없어요.'}</p>}</article>
          </div>
          <article className="next-cue"><span>NEXT</span><strong>{next ? `${next.scene_no}. ${next.title}` : 'Curtain Call'}</strong>{next && <div className="next-prep"><div><Shirt /><b>의상 준비</b><span>{briefingNextCostumes.length ? briefingNextCostumes.map((item) => `${item.role} → ${item.name}`).join(' · ') : briefingMember ? '내 체인지 없음' : '등록 없음'}</span></div><div><Package /><b>소품 준비</b><span>{briefingNextProps.length ? briefingNextProps.map((item) => `${item.name} (${item.inBy || '담당 미정'})`).join(' · ') : briefingMember ? '내 준비 업무 없음' : '등록 없음'}</span></div></div>}</article>
          <div className="show-actions"><button disabled={showHold || !showController || !showIndex} onClick={goPreviousDuringRun}>이전</button><button className="go-button" disabled={showHold || !showController} onClick={() => void goNextWithTiming()}>{next ? 'GO' : '런 종료'} <Play fill="currentColor" /></button></div>
        </>}
      </section>}
      {notice && <p className="notice">{notice}</p>}
    </main>
...
function ConnectedOverview({ progress, scenes, castMembers, propItems, musicByScene, open }) {
  const assignments = castMembers.filter(isCastAssignment)
  const actorGroups = [...assignments.reduce((map, member) => {
    const key = canonicalActor(member.name) || member.id
  ...
  const sceneConnections = scenes.map((scene) => {
    const cast = assignments.filter((member) => (member.sceneNumbers || []).some((number) => Number(number) === Number(scene.scene_no)))
  ...
  return <section className="connected-overview">
    ...
    <section className="connection-issues"><div className="connected-title"><div><span>LINK CHECK</span><h2>연결 확인</h2></div><button className="organize-action" onClick={() => open('import')}><span><WandSparkles /> 자동정리</span><ChevronRight /></button></div>{issues.length ? <div>{issues.map((issue) => <button key={issue.key} onClick={() => open(issue.tab)}><AlertTriangle /><span>{issue.label}</span><b>{issue.count}</b><ChevronRight /></button>)}</div> : <p><CheckCircle2 /> 모든 핵심 자료가 장면에 연결되어 있어요.</p>}</section>
...
function _PreparationHealth({ alerts, open }) {
...
function RunControl({ session, elapsed, history, current, firstScene, firstProps = [], firstCostumes = [], pairName, start, enabled }) {
  ...
  return <section className={session ? 'full-run-control running' : 'full-run-control'}>
    ...
    </> : <div className="run-live"><span>완료 장면 <b>{session.segments.length}</b></span><span>현재 장면 <b>{formatDuration(Math.max(0, Math.floor((Date.now() - new Date(session.sceneStartedAt).getTime()) / 1000)))}</b></span></div>}
...
function BackupPanel({ workspace, production, scenes, castMembers, propItems, musicByScene, restore, busy }) {
  ...
  async function exportBackup() {
    const backup = {
      ...
      production,
      scenes: scenes.map((scene) => ({ ...scene, costumes: parseSceneCostumes(scene.summary).map(withoutRawLine), cues: parseSceneCues(scene.summary).map(withoutRawLine) })),
      castMembers,
      props: propItems,
...
function ProductionTeamPanel({ production, session, castMembers, invite, changeMyRole, busy }) {
  const assignments = castMembers.filter(isCastAssignment)
  const [members, setMembers] = useState([])
  const myCasts = assignments.filter((member) => member.userId === session.user.id)
  ...
  useEffect(() => {
    let active = true
    supabase.from('production_members').select('user_id, role').eq('production_id', production.id).then(({ data, error }) => {
      if (!active) return
      if (error && isMissingBackendObject(error, 'production_members')) {
        setMembers([{ user_id: session.user.id, role: production.created_by === session.user.id ? 'owner' : 'member' }])
      } else setMembers(data || [])
    })
    return () => { active = false }
  }, [production.created_by, production.id, session.user.id])
  const claimedRoles = assignments.filter((member) => member.userId)
  ...
  const unavailableActors = new Set(assignments.filter((member) => member.userId && member.userId !== session.user.id).map((member) => canonicalActor(member.name)))
  const availableRoles = assignments.filter((member) => !unavailableActors.has(canonicalActor(member.name)) && (!member.userId || member.userId === session.user.id)).reduce((groups, member) => {
    const key = canonicalActor(member.name) || member.id
    const group = groups.find((item) => item.key === key)
...
function ProductionDangerPanel({ workspace, production, session, castMembers, clearUploads, deleteProduction, busy }) {
  ...
  const refresh = useCallback(async () => {
    const [{ data: memberRows, error: memberError }, { data: requestFile }] = await Promise.all([
    ...
    if (memberError && !isMissingBackendObject(memberError, 'production_members')) setStatus(`참여 팀원 확인 실패: ${memberError.message}`)
  ...
  }, [currentUserId, production.created_by, production.id, requestPath])
  ...
  async function approveDeletion() {
    ...
    if (allApproved && production.created_by === currentUserId) await deleteProduction(production.id, { skipConfirm: true })
    else if (allApproved) setStatus('전원 승인이 완료됐어요. 공연을 만든 계정이 최종 삭제할 수 있어요.')
  ...
  const allApproved = Boolean(request && requiredIds.length && requiredIds.every((id) => request.approvals?.[id]))
  const canFinalizeDeletion = allApproved && production.created_by === currentUserId
  function approvalIdentity(userId, index) {
    const cast = castMembers.find((member) => member.userId === userId)
    if (userId === currentUserId) return cast ? `나 · ${cast.roleName || cast.name}` : '나'
    return cast ? `${cast.name} · ${cast.roleName || '배역 미정'}` : `팀원 ${index + 1}`
  }
...
function ProductionMoreSheet({ active, setup, close, choose }) {
  const items = [
    ...
    { id: 'music', group: '공연 구성', label: '넘버·음악', description: '사운드트랙 자동 매칭과 순서', icon: <FileAudio /> },
...
function MusicTrackRow({ file, files, index, sceneNo, scenes = [], reorder, moveMusic, remove, loading, allowSceneChange = false }) {
  ...
  return <div className="audio-row" data-track-path={file.path}>
    <div className="audio-file-head">
      <span className="track-position" aria-label={`${index + 1}번 순서`}>{index + 1}</span>
      <FileAudio size={17} />
      <span className="track-name">{cleanStoredFileName(file.name)}</span>
      <div className="track-order-buttons" aria-label={`${cleanStoredFileName(file.name)} 순서 변경`}>
...
function SceneMusicEditor({ scene, scenes, files, reorder, moveMusic, remove, loading, openMusic }) {
  return <section className="scene-music-section">
    <div className="scene-detail-title"><Music /><strong>연결 음악</strong><span>{files.length}</span><button type="button" onClick={openMusic}><Plus /> 추가·연결</button></div>
    {files.length ? <div className="scene-music-list">{files.map((file, index) => <MusicTrackRow key={file.path} file={file} files={files} index={index} sceneNo={scene.scene_no} scenes={scenes} reorder={reorder} moveMusic={moveMusic} remove={remove} loading={loading} allowSceneChange />)}</div> : <p className="scene-section-empty">이 장면에 연결된 음악이 없어요.</p>}
  </section>
}
function SceneCard({ scene, musicFiles = [], open, moveUp, moveDown, canMoveUp, canMoveDown, orderLoading }) {
  const sections = parseSceneSummarySections(scene.summary)
  const castCount = sections.main.length + sections.ensemble.length
  const hasLinkedDetails = castCount > 0 || sections.props.length > 0 || musicFiles.length > 0
  return <article className="scene-hub-card">
    <button className="scene-hub-head" onClick={open}><span className="scene-hub-number">{scene.scene_no}</span><div><small>ACT {scene.act_no}</small><h3>{scene.title}</h3>{hasLinkedDetails ? <p className="scene-hub-meta">{castCount > 0 && <span><Users /> {castCount}명</span>}{sections.props.length > 0 && <span><Package /> {sections.props.length}개</span>}{musicFiles.length > 0 && <span><Music /> {musicFiles.length}곡</span>}</p> : <p className="scene-hub-empty-meta">연결된 정보 없음</p>}</div><ChevronRight /></button>
  ...
  return <EntityEditorPage eyebrow={`ACT ${scene.act_no} · SCENE ${scene.scene_no}`} title={scene.title} description="배역·음악·소품과 장면 메모를 한곳에서 확인합니다." back={back}>
    <div className="scene-detail-page">
      <section className="scene-cast-sections">
        {sections.main.length > 0 && <ScenePeopleRow label="메인 배역" tone="main" people={sections.main} />}
        {sections.ensemble.length > 0 && <ScenePeopleRow label="등장 앙상블" tone="ensemble" people={sections.ensemble} />}
      </section>
      {(sections.details.length > 0 || sections.soundtracks.length > 0) && <section className="scene-hierarchy-section">{sections.details.length > 0 && <div><span>SCENE DETAIL</span>{sections.details.map((item) => <b key={item}>{item}</b>)}</div>}{sections.soundtracks.length > 0 && <div><span>SOUNDTRACK</span>{sections.soundtracks.map((item) => <b key={item}><Music />{item}</b>)}</div>}</section>}
...
function SceneEditPage({ scene, update, back }) {
  ...
  async function save(event) {
    ...
    if (await update(scene.id, draft)) back()
  }
...
function ImportPanel({ workspace, production, updateProduction, scenes, castMembers, text, setText, rows, setRows, analyze, analyzeWithAI, save, readPdf, readSpreadsheet, undo, loading, aiAnalyzing, pdfExtractionReport }) {
  ...
  return <section className="import-panel import-panel-v2">
    <header className="import-clean-head"><span>AUTO ORGANIZE</span><h2>자료 자동정리</h2><p>가져올 자료 형식을 하나 선택하세요. 분석 전에는 공연 데이터가 변경되지 않아요.</p></header>
    <nav className="import-progress" aria-label="자료 자동정리 진행 단계">{[['1','자료 선택'],['2','내용 확인'],['3','저장']].map(([number, label], index) => <span className={importStep >= index ? 'active' : ''} key={number}><b>{number}</b><small>{label}</small></span>)}</nav>
...
function ImportSelection({ rows, excluded, existing, toggle, update, selectAll, clearAll }) {
  ...
  return <section className="import-selection"><div className="import-selection-head"><div><span>APPLY ROWS</span><h3>적용할 장면 선택·수정</h3></div><strong>{selected}/{rows.length}</strong></div><div className="import-selection-actions"><button onClick={selectAll}>전체 선택</button><button onClick={clearAll}>전체 해제</button></div><div className="import-selection-list">{rows.map((row, index) => { const checked = !excluded.includes(Number(row.number)); const exists = existing.has(Number(row.number)); return <article className={checked ? 'selected' : ''} key={`${row.number}-${index}`}><label aria-label={`${row.number}. ${row.title} 적용`}><input type="checkbox" checked={checked} onChange={() => toggle(row.number)} /></label><span>{row.number}</span><div><div className="import-edit-title"><input value={row.title} onChange={(event) => update(index, { title: event.target.value })} aria-label={`${row.number}번 장면 제목`} /><em className={exists ? 'existing' : 'new'}>{exists ? '기존' : '신규'}</em></div><div className="import-cast-edit-grid"><label><span>메인 배역</span><input value={row.main || ''} onChange={(event) => update(index, { main: event.target.value })} placeholder="앤더슨 / 먼로" aria-label={`${row.number}번 메인 배역`} /></label><label><span>등장 앙상블</span><input value={row.ensemble || ''} onChange={(event) => update(index, { ensemble: event.target.value })} placeholder="경찰(다더, 앤더)" aria-label={`${row.number}번 등장 앙상블`} /></label><label><span>백 앙상블</span><input value={row.backstage || ''} onChange={(event) => update(index, { backstage: event.target.value })} placeholder="글로리아 + 폴리" aria-label={`${row.number}번 백 앙상블`} /></label></div><small className="delimiter-help">구분자 , | / - _ + 자동 인식</small><ImportPropEditor row={row} update={(props) => update(index, { props })} /></div></article> })}</div></section>
}

function ImportPropEditor({ row, update }) {
  const [open, setOpen] = useState(false)
  const props = row.props || []
  const patch = (index, value) => update(props.map((item, itemIndex) => itemIndex === index ? { ...item, ...value } : item))
  const remove = (index) => update(props.filter((_, itemIndex) => itemIndex !== index))
  const add = () => { update([...props, { kind: '소품', name: '', inBy: '', outBy: '', note: '' }]); setOpen(true) }
  return <div className={open ? 'import-prop-editor open' : 'import-prop-editor'}><div className="import-prop-editor-summary"><button type="button" onClick={() => setOpen((value) => !value)}><Package size={15} /><span>소품·대도구 {props.length}개</span><ChevronRight size={15} /></button><button type="button" className="add-prop" onClick={add}><Plus size={15} /> 추가</button></div>{open && <div className="import-prop-editor-list">{props.length ? props.map((item, index) => <article key={`${index}-${item.name}`}><div className="prop-kind-name"><select value={item.kind || '소품'} onChange={(event) => patch(index, { kind: event.target.value })} aria-label={`${row.number}번 ${index + 1}번째 소품 구분`}><option>소품</option><option>대도구</option></select><input value={item.name || ''} onChange={(event) => patch(index, { name: event.target.value })} placeholder="소품명" aria-label={`${row.number}번 ${index + 1}번째 소품명`} /><button type="button" onClick={() => remove(index)} aria-label="소품 제외"><X size={16} /></button></div><div className="prop-movement-edit"><label><span>IN</span><input value={item.inBy || ''} onChange={(event) => patch(index, { inBy: event.target.value })} placeholder="담당자" /></label><label><span>OUT</span><input value={item.outBy || ''} onChange={(event) => patch(index, { outBy: event.target.value })} placeholder="담당자" /></label></div><input className="prop-note-edit" value={item.note || ''} onChange={(event) => patch(index, { note: event.target.value })} placeholder="비고" /></article>) : <p>인식된 소품이 없어요. 직접 추가할 수 있어요.</p>}</div>}</div>
}
...
function MusicPanel({ scenes, pending, musicByScene, organize, assign, upload, remove, reorder, moveMusic, loading }) {
  const [uploadOpen, setUploadOpen] = useState(false)
  const uploadedCount = Object.values(musicByScene).reduce((sum, files) => sum + files.length, 0)
  if (uploadOpen) return <EntityEditorPage eyebrow="MUSIC UPLOAD" title="음악 등록·자동연결" description="여러 파일을 선택한 뒤 장면 매칭 결과를 확인하고 저장합니다." back={() => setUploadOpen(false)}><label className="upload-zone music-upload"><FileAudio size={28} /><strong>음악파일 여러 개 선택</strong><span>MP3, M4A, WAV, AAC · 여러 파일 동시 선택 가능</span><input type="file" accept="audio/*,.mp3,.m4a,.wav,.aac" multiple disabled={loading} onChange={(event) => organize(event.target.files || [])} /></label>{!!pending.length && <div className="music-match"><div className="import-result-head"><div><p className="eyebrow">AUTO MATCH</p><h3>{pending.length}개 파일 분류 결과</h3></div><button className="primary compact" disabled={loading || !pending.some((item) => item.sceneNo !== null)} onClick={upload}><Upload size={17} /> {loading ? '업로드 중…' : '선택 파일 저장'}</button></div><div className="music-file-list">{pending.map((item, index) => <article className={item.sceneNo !== null ? 'matched' : 'unmatched'} key={`${item.file.name}-${index}`}><FileAudio /><div><strong>{item.file.name}</strong><select aria-label={`${item.file.name} 넘버 선택`} value={item.sceneNo ?? ''} onChange={(event) => assign(index, event.target.value)}><option value="">넘버 선택 안 함</option>{scenes.map((scene) => <option key={scene.id} value={scene.scene_no}>{scene.scene_no}. {scene.title}</option>)}</select></div></article>)}</div></div>}</EntityEditorPage>
  return <section className="import-panel music-panel"><div className="section-heading"><div><p className="eyebrow">NUMBER MUSIC</p><h2>음악</h2></div>{!!scenes.length && <button className="primary compact" onClick={() => setUploadOpen(true)}><Plus size={18} /> 음악</button>}</div>{!scenes.length ? <Empty icon={<ListMusic />} title="먼저 장면을 등록해주세요" description="자동정리에서 넘버표를 저장하면 음악을 자동 매칭할 수 있어요." /> : <><section className="depth-summary-card"><Music /><span><b>{uploadedCount}개 음악</b><small>장면별 재생 순서와 연결 장면을 관리합니다.</small></span></section><div className="music-library">{scenes.map((scene) => { const files = musicByScene[scene.scene_no] || []; return <article className="music-scene" key={scene.id}><div className="music-scene-head"><span>{scene.scene_no}</span><div><strong>{scene.title}</strong><small>{files.length}개 파일 · 위·아래 버튼으로 순서 변경</small></div></div>{files.length ? <div className="audio-list">{files.map((file, index) => <MusicTrackRow key={file.path} file={file} files={files} index={index} sceneNo={scene.scene_no} scenes={scenes} reorder={reorder} moveMusic={moveMusic} remove={remove} loading={loading} allowSceneChange />)}</div> : <p>등록된 음악이 없어요.</p>}</article> })}</div></>}</section>
}
...
function CastPanel({ members, pairs = [], addPair, renamePair, removePair, addRosterActor, removeRosterActor, assignRole, scenes, songs = [], propItems, form, setForm, showForm, setShowForm, submit, update, remove, toggleScene, toggleChoreography, importFromScenes, consolidate, undoCleanup, busy }) {
  ...
  const assignments = members.filter(isCastAssignment)
  const rosterRecords = members.filter(isActorRoster)
  const roleRecords = members.filter((member) => !isActorRoster(member) && member.roleName?.trim())
  const actorCount = new Set([...rosterRecords, ...assignments].map((member) => canonicalActor(member.name)).filter(Boolean)).size
  const roleCount = new Set(roleRecords.map((member) => normalizeMatch(member.roleName || '')).filter(Boolean)).size
  const linkedSceneCount = new Set(assignments.flatMap((member) => member.sceneNumbers || [])).size
  const pairGroups = [...new Set([...pairs, ...members.map((member) => member.pairGroup?.trim()).filter(Boolean)])].sort((a, b) => a.localeCompare(b, 'ko'))
  const cleanupPreview = useMemo(() => assignments.flatMap((member) => {
    const roles = splitRoleEntries(member.roleName || '')
    return roles.length > 1 ? [{ member, roles }] : []
  }), [assignments])
  const statusCounts = useMemo(() => ({
    all: roleRecords.length,
    unlinked: roleRecords.filter((member) => !(member.sceneNumbers || []).length).length,
    unclaimed: assignments.filter((member) => !member.userId).length,
    cleanup: assignments.filter((member) => splitRoleEntries(member.roleName || '').length > 1).length,
  }), [roleRecords, assignments])
  const visible = roleRecords.filter((member) => {
    ...
    const matchesQuery = !normalizeMatch(query) || normalizeMatch(`${member.name} ${member.pairGroup || ''} ${member.roleName || ''} ${member.type} ${member.notes || ''} ${memberScenes}`).includes(normalizeMatch(query))
    ...
    const matchesGender = genderFilter === 'all' || (!isRoleCandidate(member) && member.gender === genderFilter)
  ...
  async function undoRegroupRoles() {
    ...
    if (restored) setGroupNotice('마지막 이름 정리 전 상태로 되돌렸어요.')
  }
  ...
  if (pairSetupOpen) return <EntityEditorPage eyebrow="PAIR & ROSTER" title="페어·배우 명단 설정" description="페어를 만든 뒤 각 페어에 실제 배우 이름을 등록합니다." back={() => setPairSetupOpen(false)}><PairCastingManager pairs={pairGroups} members={members} addPair={addPair} renamePair={renamePair} removePair={removePair} addRosterActor={addRosterActor} removeRosterActor={removeRosterActor} busy={busy} /></EntityEditorPage>
  const editingMember = members.find((member) => member.id === editingMemberId)
  if (editingMember) return <CastEditPage member={editingMember} scenes={scenes} songs={songs} update={update} toggleScene={toggleScene} toggleChoreography={toggleChoreography} busy={busy} back={() => setEditingMemberId('')} />
  return <section className="cast-panel">
    ...
    <button className="depth-entry-card" type="button" onClick={() => setPairSetupOpen(true)}><Combine /><span><b>페어·배우 명단 설정</b><small>{pairGroups.length}페어 · {rosterRecords.length}명 등록</small></span><ChevronRight /></button>
...
function PairCastingManager({ pairs, members, addPair, renamePair, removePair, addRosterActor, removeRosterActor, busy }) {
  ...
  async function submitActor(event) {
    event.preventDefault()
    const pair = actorDraft.pair.trim()
    const name = actorDraft.name.trim()
    if (!pair || !name) return
    setStatus('배우 명단 저장 중…')
    if (await addRosterActor?.(pair, name)) {
      setActorDraft({ pair, name: '' })
      setStatus(`${pair} · ${name} 등록 완료`)
    }
  }
  return <section className="pair-casting-manager">
    <button className="pair-manager-head" onClick={() => setOpen((value) => !value)}><span><Combine /><b>초기 배우 명단</b><small>페어를 만들고 배우 이름부터 등록</small></span><strong>{pairs.length}페어 · {roster.length}명</strong><ChevronRight /></button>
    {open && <div className="pair-manager-body">
      <div className="pair-setup-guide"><b>1. 페어 등록</b><span>→</span><b>2. 페어별 배우 명단</b><span>→</span><b>3. 배역 카드에서 캐스팅</b></div>
      ...
      <form className="pair-actor-form" onSubmit={submitActor}><select required value={actorDraft.pair} onChange={(event) => setActorDraft({ ...actorDraft, pair: event.target.value })}><option value="">페어 선택</option>{pairs.map((pair) => <option key={pair}>{pair}</option>)}</select><input required value={actorDraft.name} onChange={(event) => setActorDraft({ ...actorDraft, name: event.target.value })} placeholder="배우 이름" /><button className="primary compact" disabled={busy || !actorDraft.pair || !actorDraft.name.trim()}><UserRound /> 배우 추가</button></form>
      <div className="pair-roster-board">{pairs.map((pair) => { const actors = roster.filter((member) => normalizeMatch(member.pairGroup) === normalizeMatch(pair)); return <article key={pair}><div><b>{pair}</b><small>{actors.length}명</small></div><p>{actors.map((actor) => <span key={actor.id}><UserRound />{actor.name}<button onClick={() => removeRosterActor?.(actor.id)} aria-label={`${actor.name} 명단에서 삭제`}><X /></button></span>)}{!actors.length && <em>배우 이름을 등록해 주세요.</em>}</p></article> })}</div>
      {status && <p className="pair-save-status" role="status">{status}</p>}
      <div className="pair-list">{pairs.map((pair) => {
        const assignments = members.filter((member) => isCastAssignment(member) && normalizeMatch(member.pairGroup) === normalizeMatch(pair))
        const assignedRoles = new Set(assignments.map((member) => normalizeMatch(member.roleName)).filter(Boolean))
        const missing = characters.filter((role) => !assignedRoles.has(normalizeMatch(role)))
        return <article key={pair}><div className="pair-list-head"><span><b>{pair}</b><small>{assignments.length}개 캐스팅</small></span><div><button disabled={busy} onClick={() => { const next = window.prompt('변경할 페어명', pair); if (next && next.trim() !== pair) renamePair?.(pair, next) }} aria-label={`${pair} 이름 변경`}><Pencil /></button><button disabled={busy || assignments.length > 0} onClick={() => removePair?.(pair)} aria-label={`${pair} 삭제`}><Trash2 /></button></div></div>
          <div className="pair-assignment-list">{assignments.map((member) => <span key={member.id}><b>{member.roleName || '배역 미정'}</b><ChevronRight /><em>{member.name}</em>{member.subRoleName && <small>{member.subRoleName}</small>}</span>)}</div>
          {!assignments.length && <p>아래 배역 카드에서 이 페어의 배우를 선택해 주세요.</p>}
          {!!missing.length && <div className="pair-missing"><AlertTriangle /><span>미배정 Character</span><b>{missing.slice(0, 4).join(' · ')}{missing.length > 4 ? ` 외 ${missing.length - 4}` : ''}</b></div>}
        </article>
      })}</div>
      {!pairs.length && <p className="pair-empty">A페어·B페어처럼 공연의 Run Pair를 먼저 만들어주세요.</p>}
    </div>}
  </section>
}
...
function buildCastRoleHierarchy(members = []) {
  ...
  return [...groups.values()].map((group) => {
    const assignments = group.members.filter(isCastAssignment)
    const candidates = group.members.filter(isRoleCandidate)
    return {
      ...group,
      assignments,
      candidates,
      actors: [...new Set(assignments.map((member) => member.name?.trim()).filter(Boolean))],
      pairs: [...new Set(assignments.map((member) => member.pairGroup?.trim()).filter(Boolean))],
      subRoles: [...new Set(group.members.map((member) => member.subRoleName?.trim()).filter(Boolean))],
      sceneNumbers: [...new Set(group.members.flatMap((member) => member.sceneNumbers || []))].sort((a, b) => Number(a) - Number(b)),
    }
  }).sort((a, b) => a.roleName === '배역 미정' ? 1 : b.roleName === '배역 미정' ? -1 : a.roleName.localeCompare(b.roleName, 'ko'))
}

function CastRoleGroup({ group, allPairs = [], roster = [], assignRole, scenes, songs = [], propItems, remove, toggleScene, toggleChoreography, editMember, busy, forceOpen }) {
  const [open, setOpen] = useState(false)
  const [shareStatus, setShareStatus] = useState('')
  ...
  const pairRows = [...new Set([...allPairs, ...group.pairs])].map((pair) => ({
    pair,
    members: group.assignments.filter((member) => normalizeMatch(member.pairGroup) === normalizeMatch(pair)),
    actors: roster.filter((member) => normalizeMatch(member.pairGroup) === normalizeMatch(pair)),
  }))
  const primaryCandidate = group.candidates[0] || group.assignments[0]
  async function connectActor(pair, actorName) {
    if (!actorName || !primaryCandidate) return
    await assignRole?.({
      roleName: group.roleName,
      subRoleName: group.subRoles.length === 1 ? group.subRoles[0] : '',
      type: primaryCandidate.type || '주연',
  ...
  return <section className={expanded ? 'cast-role-group role-first open' : 'cast-role-group role-first'}>
    <button className="cast-role-heading" onClick={() => setOpen((value) => !value)} aria-expanded={expanded}><div><span>ROLE</span><h3>{group.roleName}</h3>{group.subRoles.length > 0 && <small>{group.subRoles.join(' · ')}</small>}<p>{group.pairs.length}/{allPairs.length}페어 · {group.actors.length}명</p></div><strong>{group.assignments.length ? `${group.assignments.length} 캐스팅` : '배우 선택 필요'} <ChevronRight /></strong></button>
    {expanded && <div className="actor-group-body role-group-body">
      {group.candidates.length > 0 && <p className="role-candidate-note"><Sparkles /> 대본에서 찾은 배역이에요. 페어별 배우를 선택하면 장면 전체에 연결됩니다.</p>}
      {!pairRows.length && <p className="role-candidate-note"><Users /> 초기 배우 명단에서 A/B 페어와 배우 이름을 먼저 등록해 주세요.</p>}
      ...
      <div className="cast-list">{group.assignments.map((member) => <CastCard key={member.id} member={member} scenes={scenes} songs={songs} remove={remove} toggleScene={toggleScene} toggleChoreography={toggleChoreography} edit={() => editMember(member.id)} />)}</div>
    </div>}
  </section>
}
...
function groupCastMembersByActor(members, options = {}) {
  ...
  return [...groups.values()].map((roles) => {
    ...
    const claimed = roles.find((member) => member.userId)
    const representative = preferred || ready || claimed || roles[0]
    return {
      ...representative,
      userId: claimed?.userId || representative.userId || '',
      email: claimed?.email || representative.email || '',
...
function MyFeedbackInbox({ items, status, refresh }) {
  if (!items.length && !status) return null
  return <section className="feedback-inbox"><div className="feedback-inbox-head"><div><span>MY FEEDBACK</span><h2>내 런 피드백</h2><p>내가 선택한 배우에게 전달된 내용만 보여요.</p></div><button onClick={() => void refresh()} aria-label="피드백 새로고침"><RotateCcw /></button></div>{status && <p className="notice">{status}</p>}<div>{items.map((item) => <article key={item.id || `${item.runId}-${item.createdAt}`}><div><b>{item.actorName || '내 피드백'}</b><span>{item.roleName || '배역 미정'}{item.pairName ? ` · ${item.pairName}` : ''}</span></div><p>{item.message}</p><small>{item.createdAt ? new Date(item.createdAt).toLocaleString('ko-KR') : ''}</small></article>)}</div></section>
}
...
function CastCard({ member, scenes, songs = [], remove, toggleScene, toggleChoreography, edit }) {
  ...
  return <article className="cast-card">
    <div className="cast-card-head"><div className={`cast-avatar cast-${member.type}`}><UserRound /></div><div><span>{member.pairGroup || '페어 미정'} · {member.type}</span><h3>{member.name}</h3><p>{[member.roleName || '배역 미정', member.subRoleName].filter(Boolean).join(' › ')}</p></div><button className="icon-button" onClick={edit} aria-label="캐스팅 정보 수정"><Pencil size={16} /></button><button className="icon-button danger" onClick={() => remove(member.id)} aria-label="캐스팅 삭제"><Trash2 size={17} /></button></div>
...
function CastEditPage({ member, scenes, songs = [], update, toggleScene, toggleChoreography, busy, back }) {
  ...
  async function save(event) {
    ...
    if (await update(member.id, draft)) back()
  }
  return <EntityEditorPage eyebrow="CASTING" title={`${member.name} 수정`} description="배역·페어·등장 장면과 음악별 안무 참여를 관리합니다." back={back}>
    <form className="panel form-grid entity-editor-form" onSubmit={save}><div className="two-col"><label><span>배우 이름</span><input required value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} /></label><label><span>성별</span><select value={draft.gender} onChange={(event) => setDraft({ ...draft, gender: event.target.value })}><option>미지정</option><option>남</option><option>여</option></select></label></div><label><span>페어</span><input value={draft.pairGroup} onChange={(event) => setDraft({ ...draft, pairGroup: event.target.value })} /></label><div className="two-col"><label><span>1Depth 배역</span><input value={draft.roleName} onChange={(event) => setDraft({ ...draft, roleName: event.target.value })} /></label><label><span>2Depth 세부배역</span><input value={draft.subRoleName} onChange={(event) => setDraft({ ...draft, subRoleName: event.target.value })} /></label></div><label><span>배역 구분</span><select value={draft.type} onChange={(event) => setDraft({ ...draft, type: event.target.value })}><option>주연</option><option>조연</option><option>앙상블</option></select></label><label><span>메모</span><textarea value={draft.notes} onChange={(event) => setDraft({ ...draft, notes: event.target.value })} /></label><button className="primary" disabled={busy}><Save size={16} /> 기본정보 저장</button></form>
    <section className="depth-relation-section"><div><b>등장 장면</b><small>배역이 실제로 등장하는 장면을 선택하세요.</small></div><div className="depth-choice-grid">{scenes.map((scene) => { const active = selectedSceneNumbers.has(Number(scene.scene_no)); return <button type="button" className={active ? 'selected' : ''} key={scene.id} onClick={() => toggleScene(member.id, scene.scene_no)}><CheckCircle2 /><span><b>SCENE {scene.scene_no}</b><small>{scene.title}</small></span></button> })}</div></section>
    <section className="depth-relation-section"><div><b>음악별 안무 설정</b><small>이 배우만 안무에서 제외할 음악을 선택하세요.</small></div>{linkedSongs.length ? <div className="choreography-song-list">{linkedSongs.map((song) => { const excluded = choreographyExcluded.has(song.key); return <button type="button" className={excluded ? 'excluded' : 'included'} key={song.key} onClick={() => toggleChoreography(member.id, song.key)}><span><Music /><span><b>{song.title}</b><small>{song.sceneNumbers.map((sceneNo) => `SCENE ${sceneNo}`).join(' · ')}</small></span></span><em>{excluded ? '안무 제외' : '안무 참여'}</em></button> })}</div> : <p className="scene-section-empty">등장 장면에 연결된 음악이 없어요.</p>}</section>
  </EntityEditorPage>
}
...
function CostumePanel({ scenes, castMembers, updateScene }) {
  ...
  async function addCostume(event) {
    ...
    const saved = await updateScene(scene.id, { ...scene, summary })
    if (!saved) return
  ...
  if (showForm) return <EntityEditorPage eyebrow="COSTUME" title="의상·퀵체인지 등록" description="장면과 배역을 연결하면 다음 등장 준비에 자동 표시됩니다." back={() => setShowForm(false)}><form className="panel form-grid entity-editor-form" onSubmit={addCostume}><div className="two-col"><label><span>장면</span><select required value={form.sceneNo} onChange={(event) => setForm({ ...form, sceneNo: event.target.value })}><option value="">장면 선택</option>{scenes.map((scene) => <option key={scene.id} value={scene.scene_no}>{scene.scene_no}. {scene.title}</option>)}</select></label><label><span>배역</span><input required list="costume-roles" value={form.role} onChange={(event) => setForm({ ...form, role: event.target.value })} /><datalist id="costume-roles">{roles.map((role) => <option key={role} value={role} />)}</datalist></label></div><label><span>의상명</span><input required placeholder="예: LOOK 2 빨강 드레스" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label><span>체인지 메모</span><input placeholder="위치·도우미·제한시간" value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} /></label><button className="primary"><Shirt size={17} /> 의상 저장</button></form></EntityEditorPage>
  return <section className="costume-panel"><div className="section-heading"><div><p className="eyebrow">COSTUME TRACK</p><h2>의상·퀵체인지</h2></div><button className="primary compact" onClick={() => setShowForm(true)}><Plus size={18} /> 의상</button></div>{!!transitions.length && <QuickChangeBoard transitions={transitions} />}{status && <p className="notice">{status}</p>}<div className="costume-groups">{!entries.length && <Empty icon={<Shirt />} title="등록된 의상이 없어요" description="장면과 배역을 지정하면 런 화면에서 현재·다음 의상으로 표시돼요." action={() => setShowForm(true)} />}{scenes.map((scene) => { const items = entries.filter((entry) => entry.scene.id === scene.id); if (!items.length) return null; return <article key={scene.id}><div className="costume-scene-head"><span>{scene.scene_no}</span><div><strong>{scene.title}</strong><small>{items.length}개 의상</small></div></div><div>{items.map((item) => <div className="costume-row" key={`${item.role}-${item.index}`}><Shirt /><div><b>{item.role} 의상</b><strong>{item.name}</strong>{item.note && <small>{item.note}</small>}</div><button className="icon-button danger" onClick={() => removeCostume(item)} aria-label="의상 삭제"><Trash2 size={16} /></button></div>)}</div></article> })}</div></section>
}
...
function PropsEditorForm({ form, setForm, scenes, submit, busy }) {
  const selected = new Set((form.sceneNos || []).map(Number))
  const toggleScene = (sceneNo) => setForm({
    ...form,
    sceneNos: selected.has(Number(sceneNo))
      ? (form.sceneNos || []).filter((value) => Number(value) !== Number(sceneNo))
      : [...(form.sceneNos || []), Number(sceneNo)],
  })
  return <form className="panel form-grid prop-form entity-editor-form" onSubmit={submit}>
    <label className="cast-field"><span>구분</span><select value={form.kind} onChange={(event) => setForm({ ...form, kind: event.target.value })}><option>소품</option><option>대도구</option></select></label>
    <label className="cast-field"><span>이름</span><input required placeholder="소품 또는 대도구 이름" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label>
    <fieldset className="prop-scene-multi"><legend>연결할 장면 <small>복수 선택 가능</small></legend><div>{scenes.map((scene) => { const active = selected.has(Number(scene.scene_no)); return <button type="button" className={active ? 'selected' : ''} aria-pressed={active} key={scene.id} onClick={() => toggleScene(scene.scene_no)}><CheckCircle2 /><span><b>SCENE {scene.scene_no}</b><small>{scene.title}</small></span></button> })}</div>{!selected.size && <p>선택하지 않으면 ‘장면 미지정’으로 저장됩니다.</p>}</fieldset>
    <div className="two-col"><label className="cast-field"><span>IN 담당자</span><input placeholder="배우·배역" value={form.inBy} onChange={(event) => setForm({ ...form, inBy: event.target.value })} /></label><label className="cast-field"><span>OUT 담당자</span><input placeholder="배우·배역" value={form.outBy} onChange={(event) => setForm({ ...form, outBy: event.target.value })} /></label></div>
    <label className="cast-field"><span>비고</span><textarea placeholder="배치 위치, 이동 방법, 주의사항" value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} /></label>
    <button className="primary" disabled={busy}>항목 저장{selected.size > 1 ? ` · ${selected.size}개 장면` : ''}</button>
  </form>
}
...
function PropsPanel({ items, scenes, form, setForm, showForm, setShowForm, filter, setFilter, submit, update, remove, toggleReady, importFromScenes, busy }) {
  ...
  if (editingItem) return <PropEditPage item={editingItem} scenes={scenes} update={update} busy={busy} back={() => setEditingItemId('')} />
  return <section className="props-panel">
    ...
    <div className="prop-scene-groups">{!visible.length && <Empty icon={items.length ? <Search /> : <Package />} title={items.length ? '조건에 맞는 항목이 없어요' : '등록된 항목이 없어요'} description={items.length ? '다른 필터나 검색어를 선택해보세요.' : '직접 추가하거나 장면 자동정리 결과에서 한 번에 가져오세요.'} action={items.length ? null : () => setShowForm(true)} />}{groups.map((group) => <section key={group.key}><div className="prop-group-head"><span>{group.key === 'unassigned' ? '—' : group.key}</span><div><strong>{group.title}</strong><small>{group.items.filter((item) => item.ready).length}/{group.items.length} 준비</small></div></div><div className="prop-list">{group.items.map((item) => <PropCard key={item.id} item={item} sceneTitle={sceneTitle} remove={remove} toggleReady={toggleReady} edit={() => setEditingItemId(item.id)} />)}</div></section>)}</div>
  </section>
}

function PropCard({ item, sceneTitle, remove, toggleReady, edit }) {
  const hasScene = item.sceneNo !== null && item.sceneNo !== undefined && item.sceneNo !== ''
  return <article className={item.ready ? 'prop-card ready' : 'prop-card'}><button className="ready-toggle" onClick={() => toggleReady(item.id)} aria-label="준비 상태 변경"><CheckCircle2 /></button><div className="prop-copy"><div><span className={`prop-kind ${item.kind === '대도구' ? 'set-piece' : ''}`}>{item.kind}</span>{hasScene && <span className="prop-scene">{item.sceneNo}. {sceneTitle(item.sceneNo)}</span>}</div><h3>{item.name}</h3><div className="prop-assignees"><span><b>IN</b>{item.inBy || '미정'}</span><span><b>OUT</b>{item.outBy || '미정'}</span></div>{item.note && <p>{item.note}</p>}</div><button className="icon-button" onClick={edit} aria-label="소품 정보 수정"><Pencil size={16} /></button><button className="icon-button danger" onClick={() => remove(item.id)} aria-label="소품 삭제"><Trash2 size={17} /></button></article>
}

function PropEditPage({ item, scenes, update, busy, back }) {
  ...
  async function save(event) {
    event.preventDefault()
    if (!draft.name.trim()) return
    if (await update(item.id, draft)) back()
  }
  return <EntityEditorPage eyebrow={item.kind} title={`${item.name} 수정`} description="연결 장면과 IN·OUT 담당자를 수정합니다." back={back}><form className="panel form-grid entity-editor-form" onSubmit={save}><div className="two-col"><label><span>구분</span><select value={draft.kind} onChange={(event) => setDraft({ ...draft, kind: event.target.value })}><option>소품</option><option>대도구</option></select></label><label><span>연결 장면</span><select value={draft.sceneNo} onChange={(event) => setDraft({ ...draft, sceneNo: event.target.value })}><option value="">장면 미지정</option>{scenes.map((scene) => <option key={scene.id} value={scene.scene_no}>{scene.scene_no}. {scene.title}</option>)}</select></label></div><label><span>이름</span><input required value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} /></label><div className="two-col"><label><span>IN 담당자</span><input value={draft.inBy} onChange={(event) => setDraft({ ...draft, inBy: event.target.value })} /></label><label><span>OUT 담당자</span><input value={draft.outBy} onChange={(event) => setDraft({ ...draft, outBy: event.target.value })} /></label></div><label><span>비고</span><textarea value={draft.note} onChange={(event) => setDraft({ ...draft, note: event.target.value })} /></label><button className="primary" disabled={busy}><Save size={16} /> 변경 저장</button></form></EntityEditorPage>
}
...
function CuePanel({ scenes = [], completed = {}, toggle = () => {}, updateScene, autoLink = () => {}, busy }) {
  ...
  if (showForm) return <EntityEditorPage eyebrow="CUE" title="큐 등록" description="장면과 큐 유형, 큐사인을 연결합니다." back={() => setShowForm(false)}><form className="panel form-grid entity-editor-form" onSubmit={addCue}><div className="two-col"><label><span>장면</span><select required value={form.sceneNo} onChange={(event) => setForm({ ...form, sceneNo: event.target.value })}><option value="">장면 선택</option>{scenes.map((scene) => <option key={scene.id} value={scene.scene_no}>{scene.scene_no}. {scene.title}</option>)}</select></label><label><span>큐 유형</span><select value={form.type} onChange={(event) => setForm({ ...form, type: event.target.value })}><option>조명</option><option>음향</option><option>영상</option><option>무대</option><option>마이크</option></select></label></div><label><span>큐 내용</span><input required value={form.label} onChange={(event) => setForm({ ...form, label: event.target.value })} placeholder="예: 음악 시작" /></label><label><span>큐사인</span><input value={form.trigger} onChange={(event) => setForm({ ...form, trigger: event.target.value })} placeholder="선택 입력" /></label><button className="primary">큐 추가</button></form></EntityEditorPage>
  return <section className="cue-panel"><div className="section-heading"><div><p className="eyebrow">CUE SHEET</p><h2>큐시트</h2></div><div className="cue-heading-actions"><span className="cue-progress">{done}/{total} 완료</span><button className="primary compact" onClick={() => setShowForm(true)}><Plus size={17} /> 큐</button></div></div><button className="cue-auto-link" disabled={busy || !scenes.length} onClick={autoLink}><WandSparkles /><span><b>{busy ? '큐 연결 중…' : '대본·음악·배역 자동 연결'}</b><small>음악 GO, 배역 등장 준비, 대본 큐사인을 장면별로 합칩니다.</small></span><ChevronRight /></button>{!groups.length ? <Empty icon={<ListChecks />} title="등록된 큐가 없어요" description="자동 연결을 실행하거나 첫 큐를 직접 등록하세요." action={autoLink} /> : <div className="cue-groups">{groups.map(({ scene, cues }) => <article key={scene.id}><div className="cue-scene-head"><span>{scene.scene_no}</span><div><strong>{scene.title}</strong><small>{cues.length}개 큐</small></div></div><CueList cues={cues} sceneNo={scene.scene_no} completed={completed} toggle={toggle} remove={(cue) => removeCue(scene, cue)} /></article>)}</div>}</section>
}
...
function buildActorCostumeChanges(scenes, assignments) {
  const previousByActor = new Map()
  const changes = []
  ;[...scenes].sort((a, b) => Number(a.scene_no) - Number(b.scene_no)).forEach((scene) => {
    const sceneAssignments = assignments.filter((member) => (member.sceneNumbers || []).some((number) => Number(number) === Number(scene.scene_no)))
    const byActor = sceneAssignments.reduce((map, member) => {
      const key = canonicalActor(member.name) || member.id
...
// Legacy 화면은 이전 자료 읽기 호환을 위해 유지합니다.
  ...
  async function uploadMaterials(fileList) {
    ...
    try {
      const uploadOne = async (file) => {
        try {
          const path = `${base}/${targetCategory}/${safeStorageFileName(file.name)}`
...
// eslint-disable-next-line no-unused-vars
function TasksPanel({ workspace, production, castMembers, session }) {
  const [tasks, setTasks] = useState([])
  const [showForm, setShowForm] = useState(false)
  const [form, setForm] = useState({ title: '', category: '연출', assignee: '', assigneeId: '', dueDate: '' })
  const [status, setStatus] = useState('')
  ...
  async function addTask(event) {
    ...
    const selectedAssignee = castMembers.find((member) => member.id === form.assigneeId)
    const assignee = selectedAssignee ? `${selectedAssignee.roleName || '배역 미정'} · ${selectedAssignee.name}` : form.assignee.trim()
    const task = { id: crypto.randomUUID(), ...form, title: form.title.trim(), assignee, assigneeUserId: selectedAssignee?.userId || '', done: false, createdAt: new Date().toISOString() }
    if (await persist([...tasks, task], '할 일을 추가했어요.')) {
      setForm({ title: '', category: form.category, assignee: '', assigneeId: '', dueDate: '' })
      setShowForm(false)
    }
  }
  ...
  const visibleTasks = ordered.filter((task) => filter === 'all' || (filter === 'pending' && !task.done) || (filter === 'mine' && (task.assigneeUserId === session.user.id || castMembers.some((member) => member.userId === session.user.id && assignmentMatches(task.assignee, member)))))
  const myTaskCount = ordered.filter((task) => task.assigneeUserId === session.user.id || castMembers.some((member) => member.userId === session.user.id && assignmentMatches(task.assignee, member))).length
  const done = tasks.filter((task) => task.done).length
  async function shareTasks() {
    const labels = { pending: '미완료 업무', mine: '내 할 일', all: '전체 업무' }
    const lines = visibleTasks.map((task) => `${task.done ? '✅' : '⬜'} ${task.title}${task.assignee ? ` · ${task.assignee}` : ''}${task.dueDate ? ` · ${formatTaskDue(task.dueDate)}` : ''}`)
    const text = [`[StageFlow · ${production.title}]`, labels[filter], '', ...(lines.length ? lines : ['해당하는 업무가 없습니다.'])].join('\n')
    try { if (navigator.share) await navigator.share({ title: `${production.title} ${labels[filter]}`, text }); else await navigator.clipboard.writeText(text); setStatus(navigator.share ? '업무 목록을 공유했어요.' : '업무 목록을 복사했어요.') } catch (error) { if (error?.name !== 'AbortError') setStatus('업무 목록을 공유하지 못했어요.') }
  }
  return <section className="tasks-panel"><div className="section-heading"><div><p className="eyebrow">PRODUCTION CHECKLIST</p><h2>공연 준비 할 일</h2></div><button className="primary compact" onClick={() => setShowForm((value) => !value)}><Plus size={17} /> 할 일</button></div><section className="task-summary"><article><strong>{tasks.length - done}</strong><span>남은 일</span></article><article><strong>{done}</strong><span>완료</span></article><article><strong>{tasks.length ? Math.round((done / tasks.length) * 100) : 0}%</strong><span>완료율</span></article></section><div className="task-filter-row"><div className="task-filters"><button className={filter === 'pending' ? 'active' : ''} onClick={() => setFilter('pending')}>미완료 {tasks.length - done}</button><button className={filter === 'mine' ? 'active' : ''} onClick={() => setFilter('mine')}>내 할 일 {myTaskCount}</button><button className={filter === 'all' ? 'active' : ''} onClick={() => setFilter('all')}>전체 {tasks.length}</button></div><button className="share-task-list" onClick={shareTasks} aria-label="현재 업무 목록 공유"><Upload /></button></div>{showForm && <form className="panel task-form" onSubmit={addTask}><input required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="해야 할 일" /><div className="two-col"><select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })}><option>연출</option><option>배우</option><option>음악</option><option>의상</option><option>소품</option><option>무대</option><option>홍보</option><option>기타</option></select><select value={form.assigneeId} onChange={(event) => setForm({ ...form, assigneeId: event.target.value, assignee: '' })}><option value="">담당자 미정</option>{castMembers.map((member) => <option key={member.id} value={member.id}>{member.roleName || '배역 미정'} · {member.name}</option>)}</select></div><input type="date" value={form.dueDate} onChange={(event) => setForm({ ...form, dueDate: event.target.value })} /><button className="primary">저장</button></form>}{status && <p className="notice">{status}</p>}<div className="task-list">{!tasks.length && <Empty icon={<CheckCircle2 />} title="등록된 할 일이 없어요" description="공연 준비 업무를 등록하고 팀과 진행상황을 공유하세요." action={() => setShowForm(true)} />}{tasks.length > 0 && !visibleTasks.length && <Empty icon={<CheckCircle2 />} title="해당하는 할 일이 없어요" description="다른 필터를 선택하거나 새 할 일을 등록해보세요." />}{visibleTasks.map((task) => <TaskCard key={task.id} task={task} production={production} toggle={toggleTask} update={updateTask} remove={removeTask} />)}</div></section>
}

function TaskCard({ task, production, toggle, update, remove }) {
  ...
  const dueState = !task.done && task.dueDate ? (task.dueDate < new Date().toISOString().slice(0, 10) ? 'overdue' : task.dueDate === new Date().toISOString().slice(0, 10) ? 'due-today' : '') : ''
  function addTaskToCalendar() { if (task.dueDate) shareCalendarEvent({ id: `task-${task.id}`, date: task.dueDate, title: task.title, type: `${production.title} 할 일`, location: production.venue || '', note: [task.assignee ? `담당: ${task.assignee}` : '', `분류: ${task.category}`, 'StageFlow 공연 준비 업무'].filter(Boolean).join('\n'), reminderMinutes: 1440 }) }
  return <article className={['task-card', task.done ? 'done' : '', dueState].filter(Boolean).join(' ')}><button className="task-check" onClick={() => toggle(task.id)} aria-label="완료 상태 변경"><CheckCircle2 /></button><div><div><span>{task.category}</span>{task.dueDate && <small>{formatTaskDue(task.dueDate)}</small>}</div><strong>{task.title}</strong><p>{task.assignee ? `담당 ${task.assignee}` : '담당자 미정'}</p></div>{task.dueDate && <button className="icon-button task-calendar" onClick={addTaskToCalendar} aria-label="마감일을 캘린더에 추가"><CalendarDays size={15} /></button>}<button className="icon-button" onClick={() => setEditing(true)} aria-label="할 일 수정"><Pencil size={15} /></button><button className="icon-button danger" onClick={() => remove(task.id)} aria-label="할 일 삭제"><Trash2 size={16} /></button></article>
}
...
// eslint-disable-next-line no-unused-vars
function SchedulePanel({ workspace, production, scenes, castMembers, propItems, musicByScene }) {
  ...
  async function saveEvent(event) {
    ...
    const clean = { ...form, title: form.title.trim(), location: form.location.trim(), note: form.note.trim() }
  ...
  async function shareCallSheet(item) {
    const selected = new Set((item.sceneNumbers || []).map(Number))
    const callScenes = scenes.filter((scene) => selected.has(Number(scene.scene_no)))
    const callCast = castMembers.filter((member) => (member.sceneNumbers || []).some((number) => selected.has(Number(number))))
    const callProps = propItems.filter((prop) => selected.has(Number(prop.sceneNo)))
    const callMusic = [...selected].flatMap((sceneNo) => musicByScene[sceneNo] || musicByScene[String(sceneNo)] || [])
    const callCostumes = scenes.filter((scene) => selected.has(Number(scene.scene_no))).flatMap((scene) => parseSceneCostumes(scene.summary).map((costume) => ({ ...costume, sceneNo: scene.scene_no, key: `${scene.scene_no}-${costume.role}-${costume.name}` })))
    const lines = [
      `[StageFlow 연습 콜] ${production.title}`,
      `${item.date}${formatScheduleTimeRange(item) ? ` · ${formatScheduleTimeRange(item)}` : ''}`,
      item.location ? `장소: ${item.location}` : '장소: 미정',
      '',
      `연습 장면 (${callScenes.length})`,
      ...(callScenes.length ? callScenes.map((scene) => `- ${scene.scene_no}. ${scene.title}`) : ['- 전체 또는 미지정']),
      '',
      `호출 배우·배역 (${callCast.length})`,
      ...(callCast.length ? callCast.map((member) => `- ${member.roleName || '배역 미정'} · ${member.name}`) : ['- 지정 없음']),
      '',
      `필요 소품 (${callProps.length})`,
      ...(callProps.length ? callProps.map((prop) => `- ${prop.name}${prop.inBy ? ` · IN ${prop.inBy}` : ''}`) : ['- 지정 없음']),
      '',
      `음악 파일 (${callMusic.length})`,
      ...(callMusic.length ? callMusic.map((file) => `- ${cleanStoredFileName(file.name)}`) : ['- 지정 없음']),
      '',
      `의상·퀵체인지 (${callCostumes.length})`,
      ...(callCostumes.length ? callCostumes.map((costume) => `- ${costume.sceneNo}. ${costume.role} · ${costume.name}${costume.note ? ` · ${costume.note}` : ''}`) : ['- 지정 없음']),
      item.note ? `\n준비 메모\n${item.note}` : '',
    ].filter((line) => line !== '').join('\n')
    try {
      if (navigator.share) await navigator.share({ title: `${production.title} 연습 콜`, text: lines })
      else { await navigator.clipboard.writeText(lines); setStatus('연습 콜시트를 복사했어요.') }
    } catch (error) {
      if (error?.name !== 'AbortError') setStatus('콜시트를 공유하지 못했어요.')
    }
  }
  ...
  async function toggleAttendance(eventId, memberId) {
    const next = events.map((item) => {
      ...
      const attendance = { ...(item.attendance || {}), [memberId]: !item.attendance?.[memberId] }
      return { ...item, attendance, updatedAt: new Date().toISOString() }
    })
    await persist(next, '도착 상태를 저장했어요.')
  }

  async function toggleEventProp(eventId, propId) {
    const next = events.map((item) => {
      if (item.id !== eventId) return item
      const propReadiness = { ...(item.propReadiness || {}), [propId]: !item.propReadiness?.[propId] }
      return { ...item, propReadiness, updatedAt: new Date().toISOString() }
    })
    await persist(next, '소품 준비 상태를 저장했어요.')
  }

  async function toggleEventMusic(eventId, musicPath) {
    const next = events.map((item) => {
      if (item.id !== eventId) return item
      const musicReadiness = { ...(item.musicReadiness || {}), [musicPath]: !item.musicReadiness?.[musicPath] }
      return { ...item, musicReadiness, updatedAt: new Date().toISOString() }
    })
    await persist(next, '음악 준비 상태를 저장했어요.')
  }

  async function toggleEventCostume(eventId, costumeKey) {
    const next = events.map((item) => {
      if (item.id !== eventId) return item
      const costumeReadiness = { ...(item.costumeReadiness || {}), [costumeKey]: !item.costumeReadiness?.[costumeKey] }
      return { ...item, costumeReadiness, updatedAt: new Date().toISOString() }
    })
    await persist(next, '의상 준비 상태를 저장했어요.')
  }

  async function markCallReady(item) {
    const selected = new Set((item.sceneNumbers || []).map(Number))
    const callCast = castMembers.filter((member) => (member.sceneNumbers || []).some((number) => selected.has(Number(number))))
    const callProps = propItems.filter((prop) => selected.has(Number(prop.sceneNo)))
    const callMusic = [...selected].flatMap((sceneNo) => musicByScene[sceneNo] || musicByScene[String(sceneNo)] || [])
    const callCostumes = scenes.filter((scene) => selected.has(Number(scene.scene_no))).flatMap((scene) => parseSceneCostumes(scene.summary).map((costume) => ({ ...costume, key: `${scene.scene_no}-${costume.role}-${costume.name}` })))
    const attendance = Object.fromEntries(callCast.map((member) => [member.id, true]))
    const propReadiness = Object.fromEntries(callProps.map((prop) => [prop.id, true]))
    const musicReadiness = Object.fromEntries(callMusic.map((file) => [file.path, true]))
    const costumeReadiness = Object.fromEntries(callCostumes.map((costume) => [costume.key, true]))
    const next = events.map((event) => event.id === item.id ? { ...event, attendance, propReadiness, musicReadiness, costumeReadiness, updatedAt: new Date().toISOString() } : event)
    await persist(next, '이번 콜의 모든 준비를 완료했어요.')
  }

  async function resetCallReadiness(item) {
    if (!window.confirm('이 일정의 배우·소품·음악·의상 체크를 모두 초기화할까요?')) return
    const next = events.map((event) => event.id === item.id ? { ...event, attendance: {}, propReadiness: {}, musicReadiness: {}, costumeReadiness: {}, updatedAt: new Date().toISOString() } : event)
    await persist(next, '이번 콜의 준비 체크를 초기화했어요.')
  }
  ...
  return <section className="schedule-panel"><div className="section-heading"><div><p className="eyebrow">PRODUCTION CALENDAR</p><h2>일정</h2></div><button className="primary compact" onClick={() => showForm ? closeForm() : setShowForm(true)}>{showForm ? <X size={17} /> : <Plus size={17} />} {showForm ? '닫기' : '일정'}</button></div>{showForm && <form className="panel schedule-form" onSubmit={saveEvent}><div className="schedule-form-title"><strong>{editingId ? '일정 수정' : '새 일정'}</strong>{editingId && <span>내용을 고친 뒤 저장하세요.</span>}</div><input required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="일정 제목" /><div className="two-col"><select value={form.type} onChange={(event) => setForm({ ...form, type: event.target.value })}><option>연습</option><option>리허설</option><option>공연</option><option>회의</option><option>기타</option></select><input value={form.location} onChange={(event) => setForm({ ...form, location: event.target.value })} placeholder="장소" /></div><input required type="date" value={form.date} onChange={(event) => setForm({ ...form, date: event.target.value })} /><div className="schedule-time-range"><label><span>시작</span><input type="time" value={form.time} onChange={(event) => setForm({ ...form, time: event.target.value, endTime: event.target.value ? form.endTime : '' })} /></label><label><span>종료</span><input type="time" disabled={!form.time} min={form.time || undefined} value={form.endTime} onChange={(event) => setForm({ ...form, endTime: event.target.value })} /></label></div>{!!scenes.length && <div className="schedule-scene-picker"><div><strong>연습 장면</strong><button type="button" onClick={() => setForm({ ...form, sceneNumbers: form.sceneNumbers.length === scenes.length ? [] : scenes.map((scene) => scene.scene_no) })}>{form.sceneNumbers.length === scenes.length ? '전체 해제' : '전체 선택'}</button></div><div>{scenes.map((scene) => { const selected = form.sceneNumbers.some((number) => Number(number) === Number(scene.scene_no)); return <button type="button" className={selected ? 'selected' : ''} key={scene.id} onClick={() => setForm({ ...form, sceneNumbers: selected ? form.sceneNumbers.filter((number) => Number(number) !== Number(scene.scene_no)) : [...form.sceneNumbers, scene.scene_no] })}><span>{scene.scene_no}</span>{scene.title}</button> })}</div></div>}<textarea value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} placeholder="준비물, 참여 인원, 메모" /><button className="primary"><Save size={17} /> {editingId ? '수정 저장' : '일정 저장'}</button></form>}{status && <p className="notice">{status}</p>}<ScheduleGroup title="다가오는 일정" events={upcoming} scenes={scenes} castMembers={castMembers} propItems={propItems} musicByScene={musicByScene} edit={editEvent} shareCall={shareCallSheet} toggleAttendance={toggleAttendance} toggleEventProp={toggleEventProp} toggleEventMusic={toggleEventMusic} toggleEventCostume={toggleEventCostume} markReady={markCallReady} resetReady={resetCallReadiness} remove={removeEvent} empty="예정된 일정이 없어요." /><ScheduleGroup title="지난 일정" events={past.slice(0, 10)} scenes={scenes} castMembers={castMembers} propItems={propItems} musicByScene={musicByScene} edit={editEvent} shareCall={shareCallSheet} toggleAttendance={toggleAttendance} toggleEventProp={toggleEventProp} toggleEventMusic={toggleEventMusic} toggleEventCostume={toggleEventCostume} markReady={markCallReady} resetReady={resetCallReadiness} remove={removeEvent} empty="지난 일정이 없어요." /></section>
}

function ScheduleGroup({ title, events, scenes, castMembers, propItems, musicByScene, edit, shareCall, toggleAttendance, toggleEventProp, toggleEventMusic, toggleEventCostume, markReady, resetReady, remove, empty }) {
  const [expandedId, setExpandedId] = useState('')
  return <section className="schedule-group"><div className="compact-heading"><div><span>SCHEDULE</span><h2>{title}</h2></div><small>{events.length}개</small></div>{events.length ? <div className="schedule-list">{events.map((item) => {
    const selected = new Set((item.sceneNumbers || []).map(Number))
    const callCast = castMembers.filter((member) => (member.sceneNumbers || []).some((number) => selected.has(Number(number))))
    const callProps = propItems.filter((prop) => selected.has(Number(prop.sceneNo)))
    const callMusic = [...selected].flatMap((sceneNo) => musicByScene[sceneNo] || musicByScene[String(sceneNo)] || [])
    const callCostumes = scenes.filter((scene) => selected.has(Number(scene.scene_no))).flatMap((scene) => parseSceneCostumes(scene.summary).map((costume) => ({ ...costume, sceneNo: scene.scene_no, key: `${scene.scene_no}-${costume.role}-${costume.name}` })))
    const ready = callCast.filter((member) => item.attendance?.[member.id]).length
    const propsReady = callProps.filter((prop) => item.propReadiness?.[prop.id]).length
    const musicReady = callMusic.filter((file) => item.musicReadiness?.[file.path]).length
    const costumesReady = callCostumes.filter((costume) => item.costumeReadiness?.[costume.key]).length
    const totalChecks = callCast.length + callProps.length + callMusic.length + callCostumes.length
    const readyChecks = ready + propsReady + musicReady + costumesReady
    const readiness = totalChecks ? Math.round((readyChecks / totalChecks) * 100) : 0
    const expanded = expandedId === item.id
    return <article className="schedule-card" key={item.id}><div className="schedule-card-main"><div className="schedule-date"><strong>{new Date(`${item.date}T00:00:00`).getDate()}</strong><span>{new Date(`${item.date}T00:00:00`).toLocaleDateString('ko-KR', { month: 'short' })}</span></div><div className="schedule-copy"><span>{item.type}</span><strong>{item.title}</strong><p>{[formatScheduleTimeRange(item), item.location].filter(Boolean).join(' · ') || '시간·장소 미정'}</p>{!!item.sceneNumbers?.length && <div className="schedule-scene-summary"><Clapperboard /> 장면 {[...item.sceneNumbers].sort((a, b) => Number(a) - Number(b)).join(' · ')}</div>}{item.note && <small>{item.note}</small>}{totalChecks > 0 && <div className={readiness === 100 ? 'call-progress complete' : 'call-progress'}><div><span>전체 준비도</span><b>{readiness}%</b></div><i><em style={{ width: `${readiness}%` }} /></i></div>}<div className="call-readiness">{callCast.length > 0 && <button className={ready === callCast.length ? 'attendance-summary complete' : 'attendance-summary'} onClick={() => setExpandedId(expanded ? '' : item.id)}><Users /><span>배우 도착</span><b>{ready}/{callCast.length}</b><ChevronRight /></button>}{callProps.length > 0 && <button className={propsReady === callProps.length ? 'attendance-summary props complete' : 'attendance-summary props'} onClick={() => setExpandedId(expanded ? '' : item.id)}><Package /><span>소품 준비</span><b>{propsReady}/{callProps.length}</b><ChevronRight /></button>}{callMusic.length > 0 && <button className={musicReady === callMusic.length ? 'attendance-summary music complete' : 'attendance-summary music'} onClick={() => setExpandedId(expanded ? '' : item.id)}><FileAudio /><span>음악 준비</span><b>{musicReady}/{callMusic.length}</b><ChevronRight /></button>}{callCostumes.length > 0 && <button className={costumesReady === callCostumes.length ? 'attendance-summary costume complete' : 'attendance-summary costume'} onClick={() => setExpandedId(expanded ? '' : item.id)}><Shirt /><span>의상 준비</span><b>{costumesReady}/{callCostumes.length}</b><ChevronRight /></button>}</div></div><div className="schedule-actions"><button className="icon-button edit" onClick={() => edit(item)} aria-label="일정 수정"><Pencil size={16} /></button><button className="icon-button share-call" onClick={() => shareCall(item)} aria-label="콜시트 공유"><Upload size={16} /></button><button className="icon-button calendar" onClick={() => shareCalendarEvent(item)} aria-label="캘린더에 추가"><CalendarDays size={16} /></button><button className="icon-button danger" onClick={() => remove(item.id)} aria-label="일정 삭제"><Trash2 size={16} /></button></div></div>{expanded && <div className="attendance-list"><div className="call-bulk-actions"><button onClick={() => markReady(item)}><CheckCircle2 /> 전체 준비 완료</button><button onClick={() => resetReady(item)}><RotateCcw /> 초기화</button></div>{callCast.length > 0 && <h4>배우·배역</h4>}{callCast.map((member) => <button className={item.attendance?.[member.id] ? 'ready' : ''} key={member.id} onClick={() => toggleAttendance(item.id, member.id)}><CheckCircle2 /><span><b>{member.roleName || '배역 미정'}</b><small>{member.name}</small></span><strong>{item.attendance?.[member.id] ? '도착' : '대기'}</strong></button>)}{callProps.length > 0 && <h4>소품·대도구</h4>}{callProps.map((prop) => <button className={item.propReadiness?.[prop.id] ? 'ready' : ''} key={prop.id} onClick={() => toggleEventProp(item.id, prop.id)}><CheckCircle2 /><span><b>{prop.name}</b><small>{[prop.kind, prop.inBy ? `IN ${prop.inBy}` : ''].filter(Boolean).join(' · ')}</small></span><strong>{item.propReadiness?.[prop.id] ? '준비' : '대기'}</strong></button>)}{callMusic.length > 0 && <h4>넘버·음악</h4>}{callMusic.map((file) => <button className={item.musicReadiness?.[file.path] ? 'ready' : ''} key={file.path} onClick={() => toggleEventMusic(item.id, file.path)}><CheckCircle2 /><span><b>{cleanStoredFileName(file.name)}</b><small>재생 파일</small></span><strong>{item.musicReadiness?.[file.path] ? '준비' : '대기'}</strong></button>)}{callCostumes.length > 0 && <h4>의상·퀵체인지</h4>}{callCostumes.map((costume) => <button className={item.costumeReadiness?.[costume.key] ? 'ready' : ''} key={costume.key} onClick={() => toggleEventCostume(item.id, costume.key)}><CheckCircle2 /><span><b>{costume.role} · {costume.name}</b><small>{costume.sceneNo}장면{costume.note ? ` · ${costume.note}` : ''}</small></span><strong>{item.costumeReadiness?.[costume.key] ? '준비' : '대기'}</strong></button>)}</div>}</article>
...
async function shareCalendarEvent(item) {
  ...
  if (item.time) {
    ...
    const end = item.endTime ? new Date(`${item.date}T${item.endTime}:00`) : new Date(start.getTime() + (2 * 60 * 60 * 1000))
...
function cleanStoredFileName(value) {
  const encoded = value.match(/^\d{13}(?:-[A-Fa-f0-9]{8})?--([A-Za-z0-9_-]+)(\.[A-Za-z0-9]+)?$/)
  if (encoded) {
    try {
...
function _parseScriptByMarkers(source) {
...
function safeStorageFileName(value) {
  ...
  const extensionMatch = input.match(/\.([A-Za-z0-9]{1,8})$/)
  const extension = extensionMatch ? `.${extensionMatch[1].toLowerCase()}` : ''
  const rawTitle = extension ? input.slice(0, -extension.length) : input
  const title = [...rawTitle].slice(0, 120).join('').trim() || 'track'
  const bytes = new TextEncoder().encode(title)
  let binary = ''
  bytes.forEach((byte) => { binary += String.fromCharCode(byte) })
  const encoded = btoa(binary).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/g, '')
  return `${Date.now()}-${crypto.randomUUID().slice(0, 8)}--${encoded || 'track'}${extension}`
}
...
function parseProductionSheet(source) {
  ...
  const songRows = _parseScriptByMarkers(source)
  if (songRows.length) return songRows
  const rows = new Map()
  let current = null
  let propsMode = false
...
function parseScriptSceneHierarchy(source) {
  ...
  source.replace(/\r/g, '').split('\n').forEach((rawLine) => {
    ...
    if (sceneMatch) {
      ...
      if (detailNo !== null) current.details.push({ code: `${sceneNo}-${detailNo}`, title: cleanTitle(sceneMatch.title, `SCENE ${sceneNo}-${detailNo}`) })
    ...
    if (speaker && !/^(조명|음향|영상|무대|소품|의상|장소|시간)$/i.test(speaker[1].trim())) {
      ...
      current.main = current.characters.join(' / ')
    }
  })
...
function formatSceneSummary(row) {
  ...
  if (row.details?.length) {
    lines.push('Scene Detail:')
    row.details.forEach((item) => lines.push(`- SCENE #${item.code}: ${item.title || '세부 구간'}`))
  }
