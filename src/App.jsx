...
function SceneCard({ scene, update, remove }) {
  ...
  return <article className={expanded ? 'scene-hub-card open' : 'scene-hub-card'}>
    <button className="scene-hub-head" onClick={() => setExpanded((value) => !value)} aria-expanded={expanded}><span className="scene-hub-number">{scene.scene_no}</span><div><small>ACT {scene.act_no} · SCENE {scene.scene_no}</small><h3>{scene.title}</h3>{hasLinkedDetails ? <p className="scene-hub-meta">{castCount > 0 && <span><Users /> 배역 {castCount}</span>}{sections.props.length > 0 && <span><Package /> 준비물 {sections.props.length}</span>}</p> : <p className="scene-hub-empty-meta">등장인물·준비물 연결 필요</p>}</div><ChevronRight /></button>
