...
function ProductionView(props) {
  ...
  return <div className="app-shell production-shell-v2">
    ...
    <main className="content production-content">
      ...
      {tab === 'cues' && <CuePanel scenes={scenes} completed={completedCues} toggle={toggleCue} />}
...
function CuePanel({ scenes, completed, toggle }) {
  ...
  return <section className="cue-panel"><div className="section-heading"><div><p className="eyebrow">CUE SHEET</p><h2>큐시트</h2></div><span className="cue-progress">{done}/{total} 완료</span></div>{!groups.length ? <Empty icon={<ListChecks />} title="등록된 큐가 없어요" description="자동정리 또는 장면 편집에서 [조명], [음향], [영상], [무대] 형식으로 큐를 추가하세요." /> : <div className="cue-groups">{groups.map(({ scene, cues }) => <article key={scene.id}><div className="cue-scene-head"><span>{scene.scene_no}</span><div><strong>{scene.title}</strong><small>{cues.length}개 큐</small></div></div><CueList cues={cues} sceneNo={scene.scene_no} completed={completed} toggle={toggle} /></article>)}</div>}</section>
}
